...
namespace Framework.IO
{
    public static partial class ZLib
    {
        ...
        public static int DeflateInit2(z_stream? strm, int level, int method, int windowBits, int memLevel,
            int strategy)
        {
        ...
        public static int DeflateSetDictionary(z_stream? strm, byte[]? dictionary, uint dictLength)
        {
            ...
            if (
                strm?.state == null ||
                dictionary == null ||
                strm.state is not deflate_state s ||
                s.wrap == 2 ||
            ...
            {
                length = s.w_size;
                dictionaryInd = (int)(dictLength - length); // use the tail of the dictionary
            }

            //was memcpy(s.window, dictionary+dictionary_ind, length);
            Array.Copy(dictionary, dictionaryInd, s.window, 0, length);
            ...
            for (n = 0; n <= length - MIN_MATCH; n++)
            {
                ...
                hashHead = s.prev[n & s.w_mask] = s.head[s.ins_h];
                s.head[s.ins_h] = (ushort)n;
            }

            if (hashHead != 0) hashHead = 0; // to make compiler happy
            return Z_OK;
        }
        ...
        public static int deflate(z_stream strm, int flush)
        {
            ...
            if (strm.out_buf == null || (strm.in_buf == null && strm.avail_in != 0) ||
                ...
                if (s.wrap == 2)
                {
                    ...
                    if (s.gzhead == null)
                    {
                        ...
                        s.pending_buf[s.pending++] =
                            (byte)(s.level == 9 ? 2 : s.strategy >= Z_HUFFMAN_ONLY || s.level < 2 ? 4 : 0);
                    ...
                    {
                        s.pending_buf[s.pending++] =
                            (byte)((s.gzhead.text != 0 ? 1 : 0) + (s.gzhead.hcrc != 0 ? 2 : 0) +
        ...
        static uint longest_match(deflate_state s, uint cur_match)
        {
            ...
            var limit = s.strstart > s.w_size - MIN_LOOKAHEAD ? s.strstart - (s.w_size - MIN_LOOKAHEAD) : NIL;
        ...
        static uint longest_match_fast(deflate_state s, uint cur_match)
        {
            ...
            var len = MAX_MATCH - (strend_ind - scan_ind); // length of current match
                ...
                if (s.match_length >= MIN_MATCH)
                {
                        ...
                        s.lookahead >= MIN_MATCH) // max_lazy_match was max_insert_length as #define
                    {
                        ...
                        do
                        {
                            s.strstart++;
                            //was INSERT_STRING(s, s.strstart, hash_head);
                            s.ins_h = ((s.ins_h << (int)s.hash_shift) ^ s.window[s.strstart + (MIN_MATCH - 1)]) &
                                      s.hash_mask;
                            hash_head = s.prev[s.strstart & s.w_mask] = s.head[s.ins_h];
                            s.head[s.ins_h] = (ushort)s.strstart;
