...
public class GameSessionData
{
    ...
    public readonly Queue<ServerPacket> PendingUninstancedPackets = new();
    ...
    public readonly ushort[] ActiveGlyphs = new ushort[6];
    ...
    public readonly Dictionary<uint, QueryGameObjectResponse> GameObjectQueryCache = new();
    ...
    public readonly List<OwnCharacterInfo> OwnCharacters = new();
    ...
    public readonly List<ClientCastRequest> PendingClientPetCasts = new();

    public WowGuid64 LastLootTargetGuid;

    public List<int> ActionButtons = new();

    public readonly Dictionary<WowGuid128, Dictionary<byte, int>> UnitAuraDurationUpdateTime = new();

    public readonly Dictionary<WowGuid128, Dictionary<byte, int>> UnitAuraDurationLeft = new();

    public readonly Dictionary<WowGuid128, Dictionary<byte, int>> UnitAuraDurationFull = new();

    public readonly Dictionary<WowGuid128, Dictionary<byte, WowGuid128>> UnitAuraCaster = new();

    public readonly Dictionary<WowGuid128, PlayerCache> CachedPlayers = new();

    public HashSet<WowGuid128> IgnoredPlayers = new();

    public readonly Dictionary<WowGuid128, uint> PlayerGuildIds = new();

    public readonly Mutex ObjectCacheMutex = new();

    public readonly Dictionary<WowGuid128, Dictionary<int, UpdateField>> ObjectCacheLegacy = new();

    public readonly Dictionary<WowGuid128, UpdateFieldsArray>
        ObjectCacheModern = new();

    public readonly Dictionary<WowGuid128, ObjectType> OriginalObjectTypes = new();

    public readonly Dictionary<WowGuid128, uint[]> ItemGems = new();

    public readonly Dictionary<uint, Class> CreatureClasses = new();

    public readonly Dictionary<string, int> ChannelIds = new();

    public readonly Dictionary<uint, uint> ItemBuyCount = new();

    public readonly Dictionary<uint, uint> RealSpellToLearnSpell = new();

    public readonly Dictionary<uint, ArenaTeamData> ArenaTeams = new();

    public MailListResult PendingMailListPacket;

    public readonly HashSet<uint> RequestedItemTextIds = new();

    public readonly Dictionary<uint, string> ItemTexts = new();

    public readonly Dictionary<uint, uint> BattleFieldQueueTypes = new();

    public readonly Dictionary<uint, long> BattleFieldQueueTimes = new();

    public readonly Dictionary<uint, uint> DailyQuestsDone = new();

    public readonly HashSet<WowGuid128> FlagCarrierGuids = new();

    public readonly Dictionary<WowGuid64, ushort> ObjectSpawnCount = new();

    public readonly HashSet<WowGuid64> DespawnedGameObjects = new();

    public readonly HashSet<WowGuid128> HunterPetGuids = new();

    public readonly Dictionary<WowGuid128, Array<ArenaTeamInspectData>> PlayerArenaTeams = new();

    public readonly HashSet<string> AddonPrefixes = new();

    public readonly Dictionary<byte, Dictionary<byte, int>> FlatSpellMods = new();

    public readonly Dictionary<byte, Dictionary<byte, int>> PctSpellMods = new();

    public readonly Dictionary<WowGuid128, Dictionary<uint, WowGuid128>> LastAuraCasterOnTarget = new();

    public TradeSession? CurrentTrade = null;

    public readonly HashSet<uint> RequestedItemHotfixes = new();

    public readonly HashSet<uint> RequestedItemSparseHotfixes = new();

    // Buffer item CreateObjects until their templates arrive via hotfix
    public readonly Dictionary<uint, List<ObjectUpdate>> PendingItemCreates = new();
    ...
    {
        return GetCurrentGroup().PartyGUID;
    }
    ...
    public void GetAuraDuration(WowGuid128 guid, byte slot, out int left, out int full)
    {
        ...
        if (left > 0 && UnitAuraDurationUpdateTime.ContainsKey(guid) &&
    ...
    {
        if (LastAuraCasterOnTarget.ContainsKey(target) &&
            LastAuraCasterOnTarget[target].TryGetValue(spellId, out var caster))
        {
    ...
    {
        return CachedPlayers.ContainsKey(guid) && CachedPlayers[guid].Name != null ? CachedPlayers[guid].Name : null;
    }

    public WowGuid128? GetPlayerGuidByName(string name)
    {
        name = name.Trim().Replace("\0", "");
        return (from player in CachedPlayers
            where player.Value.Name == name && !WowGuid128.IsUnknownPlayerGuid(player.Key)
            select player.Key).FirstOrDefault();
    }
    ...
    public void UpdatePlayerCache(WowGuid128 guid, PlayerCache data)
    {
        ...
        if (CachedPlayers.TryAdd(guid, data)) return;
    ...
    public int GetLegacyFieldValueInt32<T>(WowGuid128 guid, T field)
    {
        ...
        return !updates.TryGetValue(fieldIndex, out var update) ? 0 : update.Int32Value;
    }
    ...
    public float GetLegacyFieldValueFloat<T>(WowGuid128 guid, T field)
    {
        ...
        return !updates.TryGetValue(fieldIndex, out var update) ? 0f : update.FloatValue;
    }
    ...
    public bool HasRangedWeapon()
    {
        ...
        var offset = LegacyVersion.AddedInVersion(ClientVersionBuild.V3_0_2_9056)
            ? 2
