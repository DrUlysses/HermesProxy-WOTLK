...
public class BnetServices
{
    public class BnetServiceHandlerInfo
    {
        ...
        public BnetServiceHandlerInfo(ServiceRequirement requirement, MethodInfo info, ParameterInfo[] parameters)
        {
            ...
            MethodCaller = info.CreateDelegate(ResponseType != null
    ...
    {
        void SendRpcMessage(uint serviceId, OriginalHash service, uint methodId, uint token,
    ...
    {
        private static readonly ConcurrentDictionary<(OriginalHash Service, uint MethodId), BnetServiceHandlerInfo>
            _serviceHandlers;
                    ...
                    foreach (var serviceAttr in methodInfo.GetCustomAttributes<ServiceAttribute>())
                    {
                        ...
                        if (_serviceHandlers.ContainsKey(key))
                        {
                            Log.Print(LogType.Error,
                                $"Tried to override ServiceHandler: {_serviceHandlers[key]} with {methodInfo.Name} (ServiceHash: {serviceAttr.ServiceHash} MethodId: {serviceAttr.MethodId})",
                                ".cctor", "BnetServices.ServiceManager.cs");
                        }
                        else
                        {
                            ...
                            {
                                _serviceHandlers[key] =
                                    new BnetServiceHandlerInfo(serviceAttr.Requirement, methodInfo, parameters);
                            }
                        }
                    }
                }
        ...
        public void Invoke(uint serviceId, OriginalHash serviceHash, uint methodId, uint requestToken,
            CodedInputStream stream)
        {
            if (!_serviceHandlers.TryGetValue((serviceHash, methodId), out var handler))
            {
                _serviceHolder.ServiceLog(LogType.Warn,
                    $"Client requested service {serviceHash}/m:{methodId} but this service is not implemented - sending OK stub");
                SendResponse(null);
                return;
            }

            if (handler.Requirement != ServiceRequirement.Always &&
                handler.Requirement != _serviceHolder.CurrentMatchingRequirement())
            {
                _serviceHolder.ServiceLog(LogType.Warn,
                    $"Client requested service {serviceHash}/m:{methodId} but with invalid state, required: {handler.Requirement} but only has {_serviceHolder.CurrentMatchingRequirement()}!");
            ...
            if (handler.ResponseType != null)
            {
                ...
                var status =
                    (BattlenetRpcErrorCode)handler.MethodCaller.DynamicInvoke(_serviceHolder, request, response);
            ...
            {
                _serviceHolder._net.SendRpcMessage(_serviceHolder._connectionPath == "WorldSocket" ? serviceId : 254u,
                    serviceHash, methodId, requestToken, status2,
                    message);
            }
        }
    }
    ...
    private BattlenetRpcErrorCode HandleGetAccountState(GetAccountStateRequest request,
        GetAccountStateResponse response)
    {
        if (!request.Options.FieldPrivacyInfo) return BattlenetRpcErrorCode.Ok;
    ...
    private BattlenetRpcErrorCode HandleGetGameAccountState(GetGameAccountStateRequest request,
        GetGameAccountStateResponse response)
    {
        ...
        if (!request.Options.FieldGameStatus) return BattlenetRpcErrorCode.Ok;
        response.State ??= new GameAccountState();
    ...
    private BattlenetRpcErrorCode HandleLogon(LogonRequest logonRequest, NoData response)
    {
        ...
        {
            ServiceLog(LogType.Error,
                "Battlenet.LogonRequest: Attempted to log in with game other than WoW (using " + logonRequest.Program +
                ")!");
        ...
        {
            ServiceLog(LogType.Error,
                $"Battlenet.LogonRequest: Attempted to log in with wrong game version (using {logonRequest.ApplicationVersion})!");
        ...
        {
            ServiceLog(LogType.Error,
                "Battlenet.LogonRequest: Attempted to log in with unsupported locale (using " + logonRequest.Locale +
                ")!");
        ...
        var externalChallenge = new ChallengeExternalRequest
        {
            ...
            Payload = ByteString.CopyFromUtf8(
                $"https://{endpoint.Address}:{endpoint.Port}/bnetserver/login/{logonRequest.Platform}/{logonRequest.ApplicationVersion}/{logonRequest.Locale}/")
        };
    ...
    {
        if (!BnetSessionTicketStorage.SessionsByTicket.TryGetValue(
                verifyWebCredentialsRequest.WebCredentials.ToStringUtf8(), out var tmpSession))
        {
        ...
        if (tmpSession.AccountInfo.IsBanned)
        {
            ...
            {
                ServiceLog(LogType.Debug,
                    "Session.HandleVerifyWebCredentials: Banned account " + tmpSession.AccountInfo.Login +
                    " tried to login!");
                return BattlenetRpcErrorCode.GameAccountBanned;
            }

            ServiceLog(LogType.Debug,
                "Session.HandleVerifyWebCredentials: Temporarily banned account " + tmpSession.AccountInfo.Login +
                " tried to login!");
        ...
        foreach (
            var gameAccountId in tmpSession.AccountInfo.GameAccounts.Values.Select(gameAccount =>
                new EntityId
                {
                    Low = gameAccount.Id,
    ...
    {
        return ModernVersion.ExpansionVersion switch
        {
            1 => "c1",
            2 => "bcc1",
            3 => "wotlk1",
    ...
    private BattlenetRpcErrorCode HandleGetAllValuesForAttribute(GetAllValuesForAttributeRequest request,
        GetAllValuesForAttributeResponse response)
    {
    ...
    private BattlenetRpcErrorCode GetRealmListTicket(Dictionary<string, Variant> Params, ClientResponse response)
    {
        ...
        if (identity != null)
        {
            var realmListTicketIdentity =
                Json.CreateObjectOrNull<RealmListTicketIdentity>(identity.BlobValue.ToStringUtf8(), split: true);
            if (realmListTicketIdentity != null)
            {
        ...
        if (clientInfo != null)
        {
            var realmListTicketClientInformation =
                Json.CreateObjectOrNull<RealmListTicketClientInformation>(clientInfo.BlobValue.ToStringUtf8(),
                    split: true);
            clientInfoOk = true;
    ...
    private BattlenetRpcErrorCode GetLastCharPlayed(Dictionary<string, Variant> Params, ClientResponse response)
    {
        ...
        (string realmName, string charName, ulong charLowerGuid, long lastLoginUnixSec) lastPlayedChar =
            rawLastPlayedChar.Value;
        GetSession().AuthClient.WaitOrRequestRealmList();
        var realm = GetSession().RealmManager.GetRealms().FirstOrDefault(r =>
            r.Name == lastPlayedChar.realmName && !r.Flags.HasFlag(RealmFlags.Offline));
        if (realm == null)
        {
        ...
        response.Attribute.AddBlob("Param_CharacterGUID",
            ByteString.CopyFrom(BitConverter.GetBytes(lastPlayedChar.charLowerGuid)));
    ...
    private BattlenetRpcErrorCode JoinRealm(Dictionary<string, Variant> Params, ClientResponse response)
    {
        ...
        return GetSession().RealmManager.JoinRealm(GetSession(), (uint)realmAddress.UintValue, GetSession().Build,
            GetRemoteIpEndPoint().Address, _clientSecret, GetSession().GameAccountInfo.Name, response);
    }
}