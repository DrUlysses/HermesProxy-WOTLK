...
public class WorldClient
{
    ...
    private void HandleArenaTeamCommandResult(WorldPacket packet)
    {
        ...
        arena.Error =
            (ArenaTeamCommandErrorModern)Enum.Parse(typeof(ArenaTeamCommandErrorModern), errorType.ToString());
        SendPacketToClient(arena);
    }
    ...
    private AuctionItem ReadAuctionItem(WorldPacket packet)
    {
        ...
        var enchantmentCount = (byte)(LegacyVersion.AddedInVersion(ClientVersionBuild.V3_0_2_9056) ? 7 :
            !LegacyVersion.AddedInVersion(ClientVersionBuild.V2_0_1_6180) ? 1 : 6);
    ...
    private void HandleEnumCharactersResult(WorldPacket packet)
    {
        ...
        for (byte i = 0; i < count; i++)
        {
            ...
            char1.Customizations = CharacterCustomizations.ConvertLegacyCustomizationsToModern(char1.RaceId,
                char1.SexId, skin, face, hairStyle, hairColor, facialHair);
        ...
        charEnum.RaceUnlockData.Add(new EnumCharactersResult.RaceUnlock(1, hasExpansion: true, hasAchievement: false,
            hasHeritageArmor: false));
        charEnum.RaceUnlockData.Add(new EnumCharactersResult.RaceUnlock(2, hasExpansion: true, hasAchievement: false,
            hasHeritageArmor: false));
        charEnum.RaceUnlockData.Add(new EnumCharactersResult.RaceUnlock(3, hasExpansion: true, hasAchievement: false,
            hasHeritageArmor: false));
        charEnum.RaceUnlockData.Add(new EnumCharactersResult.RaceUnlock(4, hasExpansion: true, hasAchievement: false,
            hasHeritageArmor: false));
        charEnum.RaceUnlockData.Add(new EnumCharactersResult.RaceUnlock(5, hasExpansion: true, hasAchievement: false,
            hasHeritageArmor: false));
        charEnum.RaceUnlockData.Add(new EnumCharactersResult.RaceUnlock(6, hasExpansion: true, hasAchievement: false,
            hasHeritageArmor: false));
        charEnum.RaceUnlockData.Add(new EnumCharactersResult.RaceUnlock(7, hasExpansion: true, hasAchievement: false,
            hasHeritageArmor: false));
        charEnum.RaceUnlockData.Add(new EnumCharactersResult.RaceUnlock(8, hasExpansion: true, hasAchievement: false,
            hasHeritageArmor: false));
        if (ModernVersion.ExpansionVersion >= 2 && LegacyVersion.ExpansionVersion >= 2)
        {
            charEnum.RaceUnlockData.Add(new EnumCharactersResult.RaceUnlock(10, hasExpansion: true,
                hasAchievement: false, hasHeritageArmor: false));
            charEnum.RaceUnlockData.Add(new EnumCharactersResult.RaceUnlock(11, hasExpansion: true,
                hasAchievement: false, hasHeritageArmor: false));
        }
    ...
    private void HandleLoginVerifyWorld(WorldPacket packet)
    {
        ...
        Log.Print(LogType.Server,
            $"[LoginVerifyWorld] Map={verify.MapID} Pos=({verify.Pos.X}, {verify.Pos.Y}, {verify.Pos.Z}) Orient={verify.Pos.Orientation}",
            "CharacterHandler.cs");
    ...
    {
        var updateData =
            new ObjectUpdate(GetSession().GameState.CurrentPlayerGuid, UpdateTypeModern.Values, GetSession())
            {
                ActivePlayerData =
                {
        ...
        var powerSlot = ClassPowerTypes.GetPowerSlotForClass(
            GetSession().GameState.GetUnitClass(GetSession().GameState.CurrentPlayerGuid), PowerType.ComboPoints);
        if (powerSlot >= 0)
        {
    ...
    private void HandleChannelNotify(WorldPacket packet)
    {
        ...
        switch (type)
        {
            ...
            {
                var flags = (ChannelFlags)(!LegacyVersion.AddedInVersion(ClientVersionBuild.V2_0_1_6180)
                    ? packet.ReadUInt32()
                    : packet.ReadUInt8());
                ...
                var joined = new ChannelNotifyJoined
                {
                    ...
                    ChannelGUID = WowGuid128.Create(HighGuidType703.ChatChannel,
                        GetSession().GameState.CurrentMapId.Value, GetSession().GameState.CurrentZoneId,
                        (ulong)channelId)
                };
    ...
    private void HandleServerChatMessageVanilla(WorldPacket packet)
    {
        ...
        if (Session.GameState.IgnoredPlayers.Contains(sender) && !chatFlags.HasFlag(ChatFlags.GM) &&
            chatType != ChatMessageTypeVanilla.Ignored)
        {
        ...
        if (ChatPkt.CheckAddonPrefix(GetSession().GameState.AddonPrefixes, ref language, ref text, ref addonPrefix))
        {
            ...
            var chat = new ChatPkt(GetSession(), chatTypeModern, text, language, sender, senderName, receiver, "",
                channelName, chatFlags, addonPrefix);
    ...
    private void HandleServerChatMessageWotLK(WorldPacket packet)
    {
        ...
        switch (chatType)
        {
            ...
            default:
                if (LegacyVersion.AddedInVersion(ClientVersionBuild.V3_0_2_9056) &&
                    packet.GetUniversalOpcode(isModern: false) == Opcode.SMSG_GM_MESSAGECHAT)
                {
        ...
        if (Session.GameState.IgnoredPlayers.Contains(sender) && !chatFlags.HasFlag(ChatFlags.GM) &&
            chatType != ChatMessageTypeWotLK.Ignored)
        {
        ...
        else
        {
            ...
            {
                var chatTypeModern =
                    (ChatMessageTypeModern)Enum.Parse(typeof(ChatMessageTypeModern), chatType.ToString());
                var chat = new ChatPkt(GetSession(), chatTypeModern, text, language, sender, senderName, receiver,
                    receiverName, channelName, chatFlags, addonPrefix, achievementId);
                SendPacketToClient(chat);
            }
        }
    }
    ...
    private void HandleHighestThreatUpdate(WorldPacket packet)
    {
        ...
        Log.Print(LogType.Debug, $"[Combat] HIGHEST_THREAT_UPDATE unit={unitGuid} (consumed, not forwarded)",
            "WorldClient.cs");
    }
    ...
    private void HandleAttackerStateUpdate(WorldPacket packet)
    {
        ...
        for (var i = 0; i < subDamageCount; i++)
        {
            ...
            if (LegacyVersion.RemovedInVersion(ClientVersionBuild.V3_0_3_9183) ||
                hitInfo.HasAnyFlag(HitInfo.FullAbsorb | HitInfo.PartialAbsorb))
            {
                subDmg.Absorbed = packet.ReadInt32();
            }

            if (LegacyVersion.RemovedInVersion(ClientVersionBuild.V3_0_3_9183) ||
                hitInfo.HasAnyFlag(HitInfo.FullResist | HitInfo.PartialResist))
            {
    ...
    private void HandleAttackSwingNotInRange(WorldPacket packet)
    {
        ...
        {
            var visibleItems = GetSession().GameState
                .GetCachedObjectFieldsLegacy(GetSession().GameState.CurrentPlayerGuid);
            var PLAYER_VISIBLE_ITEM_1_ENTRYID = LegacyVersion.GetUpdateField(PlayerField.PLAYER_VISIBLE_ITEM_1_ENTRYID);
            if (PLAYER_VISIBLE_ITEM_1_ENTRYID >= 0)
            {
                var offset = LegacyVersion.AddedInVersion(ClientVersionBuild.V3_0_2_9056) ? 2 :
                    LegacyVersion.AddedInVersion(ClientVersionBuild.V2_0_1_6180) ? 16 : 12;
                var rangedIdx = PLAYER_VISIBLE_ITEM_1_ENTRYID + 17 * offset;
                if (visibleItems != null && visibleItems.ContainsKey(rangedIdx) &&
                    visibleItems[rangedIdx].UInt32Value != 0)
                {
                    Log.Print(LogType.Debug,
                        "[Combat] Suppressing ATTACKSWING_NOTINRANGE - player has ranged weapon equipped",
                        "WorldClient.cs");
                    return;
                }
            }
    ...
    private void HandleGroupInvite(WorldPacket packet)
    {
        ...
        party.InviterRealm = new VirtualRealmInfo(realm.Id.GetAddress(), isHomeRealm: true, isInternalRealm: false,
            realm.Name, realm.NormalizedName);
    ...
    private void HandleGroupListTBC(WorldPacket packet)
    {
        ...
        if (membersCount != 0)
        {
            ...
            party.DifficultySettings.DungeonDifficultyID = (DifficultyModern)Enum.Parse(typeof(DifficultyModern),
                ((DifficultyLegacy)difficultyId /*cast due to .constrained prefix*/).ToString());
            if (ModernVersion.ExpansionVersion > 1)
            {
                party.DifficultySettings.RaidDifficultyID = DifficultyModern.Raid25N;
            }
    ...
    private void HandleGuildEvent(WorldPacket packet)
    {
        ...
        switch (eventType)
        {
            ...
            case GuildEventType.Demotion:
            {
                ...
                var rankId = GetSession().GetGuildRankIdByName(
                    GetSession().GameState.GetPlayerGuildId(GetSession().GameState.CurrentPlayerGuid), strings[2]);
        ...
        if (ranksCount > 0)
        {
            ...
            for (byte i = 0; i < ranksCount; i++)
            {
                var rank = new GuildRankData
                {
                    ...
                    RankOrder = i,
                    RankName = GetSession()
                        .GetGuildRankNameById(
                            GetSession().GameState.GetPlayerGuildId(GetSession().GameState.CurrentPlayerGuid), i),
    ...
    private void HandleItemPushResult(WorldPacket packet)
    {
        ...
        else
        {
            ...
            if (objective != null)
            {
                var updateFields = GetSession().GameState
                    .GetCachedObjectFieldsLegacy(GetSession().GameState.CurrentPlayerGuid);
                ...
                for (var i = 0; i < questsCount; i++)
                {
                    ...
                    if (logEntry != null && logEntry.QuestID.HasValue && logEntry.QuestID == objective.QuestID &&
                        logEntry.ObjectiveProgress[objective.StorageIndex].HasValue)
                    {
        ...
        if (item.Slot == byte.MaxValue && item.SlotInBag >= 0 &&
    ...
    private void HandleInventoryChangeFailure(WorldPacket packet)
    {
        ...
        if (failure.BagResult != InventoryResult.Ok)
        {
            ...
            if (
                ...
                return;
            GetSession().InstanceSocket?.SendCastRequestFailed(
                GetSession().GameState.CurrentClientNormalCast,
                isPet: false
            );
            GetSession().GameState.CurrentClientNormalCast = null;
        }
    ...
    private void HandleSellResponse(WorldPacket packet)
    {
        ...
        Log.Print(LogType.Debug, $"[SellResponse] Item={sell.ItemGUID} Vendor={sell.VendorGUID} Reason={sell.Reason}",
            "WorldClient.cs");
    ...
    private void HandleSetDungeonDifficulty(WorldPacket packet)
    {
        ...
        difficulty.DifficultyID = (byte)Enum.Parse(typeof(DifficultyModern),
            ((DifficultyLegacy)difficultyId /*cast due to .constrained prefix*/).ToString());
        packet.ReadInt32();
        packet.ReadInt32();
    ...
    private void HandleMovementMessages(WorldPacket packet)
    {
        ...
        moveUpdate.MoveInfo.Flags =
            (uint)((MovementFlagWotLK)moveUpdate.MoveInfo.Flags).CastFlags<MovementFlagModern>();
    ...
    private void HandleTransferAborted(WorldPacket packet)
    {
        ...
        else
        {
            ...
            transfer.Reason =
                (TransferAbortReasonModern)Enum.Parse(typeof(TransferAbortReasonModern), legacyReason.ToString());
        }
    ...
    private void HandleMoveForceSpeedChange(WorldPacket packet)
    {
        ...
        if (LegacyVersion.AddedInVersion(ClientVersionBuild.V2_0_1_6180) &&
            packet.GetUniversalOpcode(isModern: false) == Opcode.SMSG_FORCE_RUN_SPEED_CHANGE)
        {
            packet.ReadUInt8();
        }
    ...
    {
        var opcodeName = packet.GetUniversalOpcode(isModern: false).ToString()
            .Replace("MSG_MOVE_SET", "SMSG_MOVE_UPDATE");
        var universalOpcode = Opcodes.GetUniversalOpcode(opcodeName);
    ...
    private void HandleMonsterMove(WorldPacket packet)
    {
        ...
        if (LegacyVersion.RemovedInVersion(ClientVersionBuild.V2_0_1_6180))
        {
            ...
            if (splineFlags == SplineFlagVanilla.Runmode)
            {
                ...
                var unitFlags = (UnitFlagsVanilla)GetSession().GameState
                    .GetLegacyFieldValueUInt32(guid, UnitField.UNIT_FIELD_FLAGS);
        ...
        else if (LegacyVersion.RemovedInVersion(ClientVersionBuild.V3_0_2_9056))
        {
            ...
            {
                moveSpline.SplineFlags = SplineFlagModern.Unknown5;
                var unitFlags2 = (UnitFlags)GetSession().GameState
                    .GetLegacyFieldValueUInt32(guid, UnitField.UNIT_FIELD_FLAGS);
        ...
        else
        {
            ...
            for (var j = 1; j < moveSpline.SplineCount; j++)
            {
                ...
                vec2 = !LegacyVersion.AddedInVersion(ClientVersionBuild.V2_0_1_6180)
                    ? moveSpline.EndPosition - vec2
                    : mid - vec2;
                moveSpline.SplinePoints.Add(vec2);
            }
        }

        var isTaxiFlight = hasTaxiFlightFlags &&
                           (GetSession().GameState.IsWaitingForTaxiStart || Math.Abs(packet.GetReceivedTime() -
                               GetSession().GameState.CurrentPlayerCreateTime) <= 1000) &&
        ...
        if (isTaxiFlight)
        {
            ...
            SendPacketToClient(update);
            moveSpline.SplineFlags = SplineFlagModern.Flying | SplineFlagModern.CatmullRom | SplineFlagModern.CanSwim |
                                     SplineFlagModern.UncompressedPath | SplineFlagModern.Unknown5 |
                                     SplineFlagModern.Steering | SplineFlagModern.Unknown10;
    ...
    private void HandleTrainerBuyFailed(WorldPacket packet)
    {
        ...
        SendPacketToClient(buy);
        var chat = new ChatPkt(GetSession(), ChatMessageTypeModern.System,
            $"Failed to learn Spell {buy.SpellID} (Reason {buy.TrainerFailedReason}).");
        SendPacketToClient(chat);
    }
    ...
    private void HandlePetitionDecline(WorldPacket packet)
    {
        ...
        {
            var chat = new ChatPkt(GetSession(), ChatMessageTypeModern.System,
                name + " has declined your guild invitation.");
            SendPacketToClient(chat);
        }
    }
    ...
    private void HandleQueryGameObjectResponse(WorldPacket packet)
    {
        ...
        if (gameObject.Type == 15) // MO_TRANSPORT — log template data for debugging
            Log.Print(LogType.Debug,
                $"[GOQuery] Entry={response.GameObjectID} Type={gameObject.Type} DisplayID={gameObject.DisplayID} Name={gameObject.Name[0]} data0(pathID)={gameObject.Data[0]} data1(speed)={gameObject.Data[1]} data2(accel)={gameObject.Data[2]} Size={gameObject.Size}");
        SendPacketToClient(response);
    }
    ...
    private void HandleQueryNpcTextResponse(WorldPacket packet)
    {
        ...
        for (var i = 0; i < 8; i++)
        {
            ...
            if ((string.IsNullOrEmpty(maleText) && string.IsNullOrEmpty(femaleText)) ||
                (maleText.Equals("Greetings $N") && femaleText.Equals("Greetings $N") && i != 0))
            {
                response.BroadcastTextID[i] = 0u;
            }
            else
            {
                response.BroadcastTextID[i] =
                    GameData.GetBroadcastTextId(maleText, femaleText, language, emoteDelays, emotes);
            }
        }
    ...
    private void HandleItemQueryResponse(WorldPacket packet)
    {
        ...
        if (entry.Value)
        {
            ...
            Log.Print(LogType.Debug, $"Item #{entry.Key} not found on legacy server, skipping Invalid DBReply.",
                "QueryHandler.cs");
        }
        else
        {
            ...
            if (GetSession().GameState.PendingItemCreates.TryGetValue(itemEntryKey, out var pendingUpdates))
            {
                ...
                foreach (var pending in pendingUpdates)
                {
                    ...
                    Log.Print(LogType.Debug,
                        $"Flushing buffered item CreateObject {pending.Guid} entry={itemEntryKey} after template arrived.",
                        "");
                }
    ...
    private void HandleQuestGiverQuestDetails(WorldPacket packet)
    {
        ...
        if (quest.QuestFlags[0].HasAnyFlag(QuestFlags.HiddenRewards) &&
            LegacyVersion.RemovedInVersion(ClientVersionBuild.V3_3_5a_12340))
        {
            packet.ReadUInt32();
            packet.ReadUInt32();
    ...
    private void HandleQuestGiverQuestComplete(WorldPacket packet)
    {
        ...
        if (questTemplate != null && questTemplate.RewardNextQuest == 0)
        {
            ...
            {
                var npcFlags = GetSession().GameState
                    .GetLegacyFieldValueUInt32(GetSession().GameState.CurrentInteractedWithNPC,
                        UnitField.UNIT_NPC_FLAGS);
    ...
    private void HandleQuestUpdateAddItem(WorldPacket packet)
    {
        ...
        {
            var logEntry = ReadQuestLogEntry(i, null, updateFields);
            if (logEntry is not { QuestID: not null } ||
                GameData.GetQuestTemplate((uint)logEntry.QuestID.Value) != null) continue;
    ...
    private void HandleSendKnownSpells(WorldPacket packet)
    {
        ...
        for (ushort i = 0; i < spellCount; i++)
        {
            ...
            var spellId = !LegacyVersion.AddedInVersion(ClientVersionBuild.V3_1_0_9767)
                ? packet.ReadUInt16()
                : packet.ReadUInt32();
        ...
        if (cooldownCount != 0)
        {
            ...
            for (ushort i2 = 0; i2 < cooldownCount; i2++)
            {
                ...
                var spellId2 = !LegacyVersion.AddedInVersion(ClientVersionBuild.V3_1_0_9767)
                    ? packet.ReadUInt16()
                    : packet.ReadUInt32();
                history.SpellID = spellId2;
                var itemId = !LegacyVersion.AddedInVersion(ClientVersionBuild.V4_2_2_14545)
                    ? packet.ReadUInt16()
                    : packet.ReadUInt32();
    ...
    private void HandleCastFailed(WorldPacket packet)
    {
        ...
        else
        {
            ...
            if (GetSession().GameState.CurrentClientNormalCast?.HasStarted == false)
            {
    ...
    private void HandlePetCastFailed(WorldPacket packet)
    {
        ...
        var spellId = packet.ReadUInt32();
        var status = packet.ReadUInt8();
        if (status != 2 || GetSession().GameState.CurrentClientPetCast == null ||
    ...
    {
        var casterUnit = !LegacyVersion.AddedInVersion(ClientVersionBuild.V2_0_1_6180)
            ? packet.ReadGuid().To128(GetSession().GameState)
        ...
        else if (casterUnit == GetSession().GameState.CurrentPlayerGuid &&
                 GetSession().GameState.CurrentChanneledSpellId == spellId &&
        ...
        {
            castId = WowGuid128.Create(HighGuidType703.Cast, SpellCastSource.Normal,
                GetSession().GameState.CurrentMapId.Value, spellId, spellId + casterUnit.GetCounter());
    ...
    private void HandleSpellStart(WorldPacket packet)
    {
        ...
        if (LegacyVersion.RemovedInVersion(ClientVersionBuild.V2_0_1_6180) &&
            GameData.DispellSpells.Contains((uint)spell.Cast.SpellID))
        {
    ...
    private void HandleSpellGo(WorldPacket packet)
    {
            ...
            GetSession().GameState.CurrentClientNormalCast.SpellId == spell.Cast.SpellID)
        {
            ...
            GetSession().GameState.CurrentChanneledSpellVisualId =
                GetSession().GameState.CurrentClientNormalCast.SpellXSpellVisualId;
            GetSession().GameState.CurrentClientNormalCast = null;
        }
        ...
        if (!spell.Cast.CasterUnit.IsEmpty() && GameData.AuraSpells.Contains((uint)spell.Cast.SpellID))
        {
            ...
            {
                GetSession().GameState
                    .StoreLastAuraCasterOnTarget(target, (uint)spell.Cast.SpellID, spell.Cast.CasterUnit);
            }
        }
    ...
    private SpellCastData HandleSpellStartOrGo(WorldPacket packet, bool isSpellGo)
    {
        ...
        dbdata.CastID = WowGuid128.Create(HighGuidType703.Cast, SpellCastSource.Normal,
            GetSession().GameState.CurrentMapId.Value, (uint)dbdata.SpellID,
            (ulong)dbdata.SpellID + dbdata.CasterUnit.GetCounter());
        if (LegacyVersion.AddedInVersion(ClientVersionBuild.V2_0_1_6180) &&
            LegacyVersion.RemovedInVersion(ClientVersionBuild.V3_0_2_9056) && !isSpellGo)
        {
            packet.ReadUInt8();
        }

        var flags = dbdata.CastFlags = !LegacyVersion.AddedInVersion(ClientVersionBuild.V3_0_2_9056)
            ? packet.ReadUInt16()
            : packet.ReadUInt32();
        ...
        var targetFlags = (SpellCastTargetFlags)(LegacyVersion.AddedInVersion(ClientVersionBuild.V2_0_1_6180)
            ? packet.ReadUInt32()
            : packet.ReadUInt16());
        dbdata.Target.Flags = targetFlags;
        var unitTarget = WowGuid128.Empty;
        if (targetFlags.HasAnyFlag(SpellCastTargetFlags.CorpseMask | SpellCastTargetFlags.Unit |
                                   SpellCastTargetFlags.GameObject | SpellCastTargetFlags.UnitMinipet))
        {
    ...
    private void HandleSpellNonMeleeDamageLog(WorldPacket packet)
    {
        ...
        spell.CastID = WowGuid128.Create(HighGuidType703.Cast, SpellCastSource.Normal,
            GetSession().GameState.CurrentMapId.Value, spell.SpellID, spell.SpellID + spell.CasterGUID.GetCounter());
    ...
    private void ReadSingleAura(WorldPacket packet, WowGuid128 guid, AuraUpdate update)
    {
        ...
        var data = new AuraDataInfo
        {
            ...
            CastID = WowGuid128.Create(HighGuidType703.Cast, SpellCastSource.Aura,
                GetSession().GameState.CurrentMapId.Value, spellId, guid.GetCounter()),
    ...
    private void HandleGmTicketCreate(WorldPacket packet)
    {
        ...
        var flag = response == LegacyGmTicketResponse.CreateSuccess || response == LegacyGmTicketResponse.UpdateSuccess
            ? true
            : false;
        var isError = !flag;
    ...
    {
        var playerFlags = GetSession().GameState
            .GetLegacyFieldValueUInt32(GetSession().GameState.CurrentPlayerGuid, PlayerField.PLAYER_FLAGS);
        if (playerFlags.HasAnyFlag(PlayerFlags.GM))
        {
            var chat = new ChatPkt(GetSession(), ChatMessageTypeModern.System,
                "Disable GM mode before talking to taxi master or your game will freeze.");
            SendPacketToClient(chat);
            return;
        }
    ...
    private void HandleTradeStatus(WorldPacket packet)
    {
        ...
        switch (trade.Status)
        {
            case TradeStatus.Proposed:
                ...
                trade.PartnerAccount =
                    tradeSession.PartnerAccount = GetSession().GetGameAccountGuidForPlayer(trade.Partner);
                break;
        ...
        if (LegacyVersion.AddedInVersion(ClientVersionBuild.V2_0_1_6180))
        {
            ...
            if (actualTradeId != trade.Id)
            {
                Log.Print(LogType.Error,
                    $"Got SMSG_TRADE_STATUS_EXTENDED with wrong tradeId (expected {trade.Id} but got {actualTradeId})",
                    "TradeHandler.cs");
                return;
            }
        }
    ...
    private void HandleUpdateObject(WorldPacket packet)
    {
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            switch (type)
            {
                case UpdateTypeLegacy.Values:
                {
                    ...
                    if (guid3 == GetSession().GameState.CurrentPlayerGuid && GetSession().GameState.NeedPlayerRecreate)
                    {
                        ...
                        if (cachedFields != null && GetSession().GameState.LastSelfPlayerMoveInfo != null)
                        {
                            ...
                            StoreObjectUpdate(guid3, ObjectType.ActivePlayer, fullMask, cachedFields, createAuraUpdate,
                                null, true, createUpdate, fullMask);
                            ...
                            Log.Print(LogType.Debug,
                                $"[DeathRevive] Created full CreateObject2 with {cachedFields.Count} fields, pos={createUpdate.CreateData.MoveInfo.Position}",
                                "");
                        }
                        else
                        {
                            Log.Print(LogType.Error,
                                "[DeathRevive] Cannot recreate player — missing cached fields or MoveInfo, sending Values update instead",
                                "");
                    ...
                    if (updateData2.ObjectData != null && (updateData2.ObjectData.EntryID.HasValue ||
                                                           updateData2.ObjectData.DynamicFlags.HasValue ||
                                                           updateData2.ObjectData.Scale.HasValue))
                        hasAnythingToSend = true;
                    if (updateData2.UnitData != null &&
                        (updateData2.UnitData.Health.HasValue || updateData2.UnitData.MaxHealth.HasValue ||
                    ...
                    if (updateData2.UnitData != null && updateData2.UnitData.Power != null)
                        for (var p = 0; p < updateData2.UnitData.Power.Length; p++)
                            if (updateData2.UnitData.Power[p].HasValue)
                            {
                                hasAnythingToSend = true;
                                break;
                            }
                    ...
                    if (updateData2.UnitData != null && updateData2.UnitData.MaxPower != null)
                        for (var p = 0; p < updateData2.UnitData.MaxPower.Length; p++)
                            if (updateData2.UnitData.MaxPower[p].HasValue)
                            {
                                hasAnythingToSend = true;
                                break;
                            }
                    ...
                    if (updateData2.UnitData != null)
                    {
                        ...
                        if (u.ResistanceBuffModsPositive != null)
                            for (var r = 0; r < 7; r++)
                                if (u.ResistanceBuffModsPositive[r].HasValue)
                                {
                                    hasAnythingToSend = true;
                                    break;
                                }
                        ...
                        if (u.ResistanceBuffModsNegative != null)
                            for (var r = 0; r < 7; r++)
                                if (u.ResistanceBuffModsNegative[r].HasValue)
                                {
                                    hasAnythingToSend = true;
                                    break;
                                }
                    ...
                    if (updateData2.PlayerData != null)
                    {
                        ...
                        if (pd.PlayerFlags.HasValue || pd.PlayerFlagsEx.HasValue || pd.ChosenTitle.HasValue ||
                            pd.GuildTimeStamp.HasValue)
                            hasAnythingToSend = true;
                        if (pd.QuestLog != null)
                            for (var q = 0; q < pd.QuestLog.Length; q++)
                                if (pd.QuestLog[q] != null && pd.QuestLog[q].QuestID.HasValue)
                                {
                                    hasAnythingToSend = true;
                                    break;
                                }
                        ...
                        if (guid3 == GetSession().GameState.CurrentPlayerGuid && updateData2.UnitData != null)
                        {
                            ...
                            if (u.Stats != null)
                                for (var si = 0; si < u.Stats.Length; si++)
                                    if (u.Stats[si].HasValue)
                                        statInfo += $" Stat{si}={u.Stats[si].Value}";
                            if (u.Resistances != null)
                                for (var ri = 0; ri < 7; ri++)
                                    if (u.Resistances[ri].HasValue)
                                        statInfo += $" Res{ri}={u.Resistances[ri].Value}";
                ...
                {
                    var guid2 = LegacyVersion.AddedInVersion(ClientVersionBuild.V3_1_2_9901)
                        ? packet.ReadPackedGuid()
                        : packet.ReadGuid();
                ...
                case UpdateTypeLegacy.CreateObject1:
                {
                    ...
                    if (oldGuid2.GetHighType() == HighGuidType.Creature ||
                        oldGuid2.GetHighType() == HighGuidType.GameObject)
                    {
                        if (!GetSession().GameState.ObjectSpawnCount.ContainsKey(oldGuid2))
                        {
                            GetSession().GameState.ObjectSpawnCount.Add(oldGuid2, 0);
                        }
                        else if (oldGuid2.GetHighType() == HighGuidType.GameObject &&
                                 GetSession().GameState.DespawnedGameObjects.Contains(oldGuid2))
                        {
                    ...
                    {
                        GetSession().GameState.CurrentPlayerStorage.CompletedQuests
                            .WriteAllCompletedIntoArray(updateData4.ActivePlayerData.QuestCompleted);
                    }

                    if (guid4.IsItem() && updateData4.ObjectData.EntryID.HasValue &&
                        !GameData.ItemTemplates.ContainsKey((uint)updateData4.ObjectData.EntryID.Value))
                    {
                        ...
                        Log.Print(LogType.Debug,
                            $"Buffering item CreateObject {guid4} entry={entryId4} until template arrives.", "");
                    }
                ...
                case UpdateTypeLegacy.CreateObject2:
                {
                    ...
                    if (oldGuid.GetHighType() == HighGuidType.Creature ||
                        oldGuid.GetHighType() == HighGuidType.GameObject)
                    {
                    ...
                    if (guid == GetSession().GameState.CurrentPlayerGuid && updateData.CreateData?.MoveInfo != null)
                    {
                        ...
                        Log.Print(LogType.Debug,
                            $"[DeathRevive] Cached self player MoveInfo pos={updateData.CreateData.MoveInfo.Position}",
                            "");
                    }

                    if (guid.IsItem() && updateData.ObjectData.EntryID.HasValue &&
                        !GameData.ItemTemplates.ContainsKey((uint)updateData.ObjectData.EntryID.Value))
                    {
                        ...
                        Log.Print(LogType.Debug,
                            $"Buffering item CreateObject {guid} entry={entryId2} until template arrives.", "");
                    }
        ...
        if (otherUpdates.Count != 0 || updateObject.DestroyedGuids.Count != 0 ||
    ...
    private void ReadCreateObjectBlock(WorldPacket packet, WowGuid128 guid, ObjectUpdate updateData,
        AuraUpdate auraUpdate, object index)
    {
    ...
    public void ReadValuesUpdateBlockOnCreate(WorldPacket packet, WowGuid128 guid, ObjectType type,
        ObjectUpdate updateData, AuraUpdate auraUpdate, object index)
    {
        BitArray updateMaskArray = null;
        BitArray actuallyChangedValuesMaskArray;
        var updates = ReadValuesUpdateBlock(packet, ref type, index, isCreating: true, null, out updateMaskArray,
            out actuallyChangedValuesMaskArray);
        StoreObjectUpdate(guid, type, updateMaskArray, updates, auraUpdate, null, isCreate: true, updateData,
            actuallyChangedValuesMaskArray);
        GetSession().GameState.ObjectCacheMutex.WaitOne();
    ...
    public void ReadValuesUpdateBlock(WorldPacket packet, WowGuid128 guid, ObjectUpdate updateData,
        AuraUpdate auraUpdate, PowerUpdate powerUpdate, int index)
    {
        BitArray updateMaskArray = null;
        var type = GetSession().GameState.GetOriginalObjectType(guid);
        BitArray actuallyChangedValuesMaskArray;
        var updates = ReadValuesUpdateBlock(packet, ref type, index, isCreating: false,
            GetSession().GameState.GetCachedObjectFieldsLegacy(guid), out updateMaskArray,
            out actuallyChangedValuesMaskArray);
        StoreObjectUpdate(guid, type, updateMaskArray, updates, auraUpdate, powerUpdate, isCreate: false, updateData,
            actuallyChangedValuesMaskArray);
    ...
    private Dictionary<int, UpdateField> ReadValuesUpdateBlock(WorldPacket packet, ref ObjectType type, object index,
        bool isCreating, Dictionary<int, UpdateField>? oldValues, out BitArray outUpdateMaskArray,
        out BitArray outActuallyChangedValuesMaskArray)
    {
        ...
        if (missingCreateObject)
        {
            switch (type)
            {
                case ObjectType.Item:
                    if (mask.Count >= LegacyVersion.GetUpdateField(ItemField.ITEM_END) && maskSize ==
                        Convert.ToInt32((LegacyVersion.GetUpdateField(ContainerField.CONTAINER_END) + 32) / 32))
                    {
                ...
                case ObjectType.Player:
                    if (mask.Count >= LegacyVersion.GetUpdateField(PlayerField.PLAYER_END) && maskSize ==
                        Convert.ToInt32((LegacyVersion.GetUpdateField(ActivePlayerField.ACTIVE_PLAYER_END) + 32) / 32))
                    {
            ...
            else
            {
                switch (type)
                {
                    ...
                    case ObjectType.AzeriteItem:
                        ...
                        break;
                    case ObjectType.Player:
                        if (j < LegacyVersion.GetUpdateField(UnitField.UNIT_END) ||
                            j < LegacyVersion.GetUpdateField(UnitField.UNIT_FIELD_END))
                        {
            ...
            switch (updateFieldType)
            {
                ...
                case UpdateFieldType.Quaternion:
                {
                    ...
                    for (var quatI = 0; quatI < quaternionCount; quatI++)
                    {
                        ...
                        if (hasQuatValue)
                        {
                            PrintValue(key + (quaternionCount > 1 ? " + " + quatI : ""),
                                new Quaternion(fieldData[quatI * 4].FloatValue, fieldData[quatI * 4 + 1].FloatValue,
                ...
                case UpdateFieldType.Bytes:
                {
                    for (var num2 = 0; num2 < fieldData.Count; num2++)
                    {
                        if (mask[start + num2] && (!isCreating || fieldData[num2].UInt32Value != 0))
                        {
                            ...
                            PrintValue(num2 > 0 ? key + " + " + num2 : key,
                                intBytes[0] + "/" + intBytes[1] + "/" + intBytes[2] + "/" + intBytes[3], index);
                        }
                    }
                ...
                case UpdateFieldType.Short:
                {
                    for (var n = 0; n < fieldData.Count; n++)
                    {
                        ...
                        {
                            PrintValue(n > 0 ? key + " + " + n : key,
                                (short)(fieldData[n].UInt32Value & 0xFFFF) + "/" +
                                (short)(fieldData[n].UInt32Value >> 16), index);
                        }
                    }
                ...
                default:
                {
                    for (var m = 0; m < fieldData.Count; m++)
                    {
                        ...
                        {
                            PrintValue(m > 0 ? key + " + " + m : key,
                                fieldData[m].UInt32Value + "/" + fieldData[m].FloatValue, index);
                        }
                    }
    ...
    private void ReadMovementUpdateBlock(WorldPacket packet, WowGuid guid, ObjectUpdate updateData, object index)
    {
        ...
        var flags = (UpdateFlag)(!LegacyVersion.AddedInVersion(ClientVersionBuild.V3_1_0_9767)
            ? packet.ReadUInt8()
            : packet.ReadUInt16());
        ...
        if (flags.HasAnyFlag(UpdateFlag.Living))
        {
            ...
            if (moveFlags.HasAnyFlag(MovementFlagWotLK.SplineEnabled))
            {
                ...
                {
                    monsterMove.SplineFlags = SplineFlagModern.Flying | SplineFlagModern.CatmullRom |
                                              SplineFlagModern.CanSwim | SplineFlagModern.UncompressedPath |
                                              SplineFlagModern.Unknown5 | SplineFlagModern.Steering |
    ...
    private static WowGuid GetGuidValue<T>(Dictionary<int, UpdateField> UpdateFields, T field) where T : Enum
    {
        ...
        return new WowGuid128(MathFunctions.MakePair64(parts2[0], parts2[1]),
            MathFunctions.MakePair64(parts2[2], parts2[3]));
    }
    ...
    public QuestLog? ReadQuestLogEntry(int i, BitArray? updateMaskArray, Dictionary<int, UpdateField>? updates)
    {
        ...
        if ((updateMaskArray != null && updateMaskArray[index]) ||
            (updateMaskArray == null && updates?.ContainsKey(index) == true))
        {
            questLog ??= new QuestLog();

            questLog.QuestID = updates?[index].Int32Value;
            // Cache the QuestID for this slot
            GetSession().GameState.QuestLogQuestIDs[i] = questLog.QuestID;
            Log.Print(
                LogType.Debug,
                $"[QuestLogRead] slot={i} QuestID={questLog.QuestID} fieldIndex={index}",
                ""
            );
        }

        if ((updateMaskArray != null && updateMaskArray[index + stateOffset]) ||
            (updateMaskArray == null && updates?.ContainsKey(index + stateOffset) == true))
        {
            questLog ??= new QuestLog();
            ...
            if (LegacyVersion.RemovedInVersion(ClientVersionBuild.V2_4_0_8089))
            {
                ...
                {
                    Log.Print(LogType.Debug, $"[QuestLogRead] slot={i} QuestID={questLog.QuestID} fieldIndex={index} - Missing state update", "");
                }
            }
            else
            {
        ...
        if (progressOffset != -1 && ((updateMaskArray != null && updateMaskArray[index + progressOffset]) ||
                                     (updateMaskArray == null && updates?.ContainsKey(index + progressOffset) == true)))
        {
            questLog ??= new QuestLog();
            ...
            {
                Log.Print(LogType.Debug, $"[QuestLogRead] slot={i} QuestID={questLog.QuestID} fieldIndex={index} - Missing progress update", "");
            }
        ...
        {
            var progressOffset2 = progressOffset + 1;
            var field3Updated = (updateMaskArray != null && updateMaskArray[index + progressOffset2]) ||
                                (updateMaskArray == null && updates.ContainsKey(index + progressOffset2));
            var field2Updated = (updateMaskArray != null && updateMaskArray[index + progressOffset]) ||
    ...
    public byte ReadPvPFlags(Dictionary<int, UpdateField> updates)
    {
        ...
        if (UNIT_FIELD_FLAGS >= 0 && updates.ContainsKey(UNIT_FIELD_FLAGS) &&
            updates[UNIT_FIELD_FLAGS].UInt32Value.HasAnyFlag(UnitFlags.Pvp))
        {
            flags |= 1;
        }
    ...
    public void StoreObjectUpdate(WowGuid128 guid, ObjectType objectType, BitArray updateMaskArray,
        Dictionary<int, UpdateField> updates, AuraUpdate auraUpdate, PowerUpdate powerUpdate, bool isCreate,
        ObjectUpdate updateData, BitArray actuallyChangedValuesMaskArray)
    {
        StoreObjectUpdateInternal(guid, objectType, updateMaskArray, updates, auraUpdate, powerUpdate, isCreate,
            updateData);
        AfterStoreObjectUpdateHook(guid, objectType, updateMaskArray, updates, auraUpdate, powerUpdate, isCreate,
            updateData, actuallyChangedValuesMaskArray);
    }

    private void AfterStoreObjectUpdateHook(WowGuid128 guid, ObjectType objectType, BitArray updateMaskArray,
        Dictionary<int, UpdateField> updates, AuraUpdate auraUpdate, PowerUpdate powerUpdate, bool isCreate,
        ObjectUpdate updateData, BitArray changedValuesMask)
    {
        ...
        if (UNIT_FIELD_NATIVEDISPLAYID < 0 || UNIT_FIELD_MOUNTDISPLAYID < 0 || OBJECT_FIELD_SCALE_X < 0 ||
            (!changedValuesMask.Get(UNIT_FIELD_NATIVEDISPLAYID) && !changedValuesMask.Get(UNIT_FIELD_MOUNTDISPLAYID) &&
             !changedValuesMask.Get(OBJECT_FIELD_SCALE_X)))
        {
            return;
        }
        ...
        if (rawScaleX != 0f)
        {
            ...
            if (mountDisplayId != 0 && LegacyVersion.AddedInVersion(ClientVersionBuild.V2_0_1_6180))
            {
                ...
                calculatedBaseHeight = mountModel.MountHeight * mountDisplayInfo.DisplayScale +
                                       ourModel.Height * ourModel.ModelScale * ourDisplayInfo.DisplayScale * 0.5f;
            }
            else
            {
    ...
    private void StoreObjectUpdateInternal(WowGuid128 guid, ObjectType objectType, BitArray updateMaskArray,
        Dictionary<int, UpdateField> updates, AuraUpdate auraUpdate, PowerUpdate powerUpdate, bool isCreate,
        ObjectUpdate updateData)
    {
        ...
        {
            updateData.ObjectData.Guid =
                GetGuidValue(updates, ObjectField.OBJECT_FIELD_GUID).To128(GetSession().GameState);
        }
        ...
        if (objectType == ObjectType.Item || objectType == ObjectType.Container)
        {
            ...
            {
                updateData.ItemData.Owner =
                    GetGuidValue(updates, ItemField.ITEM_FIELD_OWNER).To128(GetSession().GameState);
            }
            ...
            {
                updateData.ItemData.ContainedIn = GetGuidValue(updates, ItemField.ITEM_FIELD_CONTAINED)
                    .To128(GetSession().GameState);
            }
            ...
            {
                updateData.ItemData.Creator =
                    GetGuidValue(updates, ItemField.ITEM_FIELD_CREATOR).To128(GetSession().GameState);
            }
            ...
            {
                updateData.ItemData.GiftCreator = GetGuidValue(updates, ItemField.ITEM_FIELD_GIFTCREATOR)
                    .To128(GetSession().GameState);
            }
            ...
            if (ITEM_FIELD_ENCHANTMENT >= 0)
            {
                ...
                if (LegacyVersion.RemovedInVersion(ClientVersionBuild.V2_0_1_6180))
                {
                    updateData.ItemData.Enchantment[EnchantmentSlot.Perm] =
                        ReadEnchantData(Enums.Vanilla.EnchantmentSlot.Perm);
                    updateData.ItemData.Enchantment[EnchantmentSlot.Temp] =
                        ReadEnchantData(Enums.Vanilla.EnchantmentSlot.Temp);
                    updateData.ItemData.Enchantment[EnchantmentSlot.Prop0] =
                        ReadEnchantData(Enums.Vanilla.EnchantmentSlot.Prop0);
                    updateData.ItemData.Enchantment[EnchantmentSlot.Prop1] =
                        ReadEnchantData(Enums.Vanilla.EnchantmentSlot.Prop1);
                    updateData.ItemData.Enchantment[EnchantmentSlot.Prop2] =
                        ReadEnchantData(Enums.Vanilla.EnchantmentSlot.Prop2);
                    updateData.ItemData.Enchantment[EnchantmentSlot.Prop3] =
                        ReadEnchantData(Enums.Vanilla.EnchantmentSlot.Prop3);
                }
                else if (LegacyVersion.RemovedInVersion(ClientVersionBuild.V3_0_2_9056))
                {
                    updateData.ItemData.Enchantment[EnchantmentSlot.Perm] =
                        ReadEnchantData(Enums.TBC.EnchantmentSlot.Perm);
                    updateData.ItemData.Enchantment[EnchantmentSlot.Temp] =
                        ReadEnchantData(Enums.TBC.EnchantmentSlot.Temp);
                    updateData.ItemData.Enchantment[EnchantmentSlot.Sock1] =
                        ReadEnchantData(Enums.TBC.EnchantmentSlot.Sock1);
                    updateData.ItemData.Enchantment[EnchantmentSlot.Sock2] =
                        ReadEnchantData(Enums.TBC.EnchantmentSlot.Sock2);
                    updateData.ItemData.Enchantment[EnchantmentSlot.Sock3] =
                        ReadEnchantData(Enums.TBC.EnchantmentSlot.Sock3);
                    updateData.ItemData.Enchantment[EnchantmentSlot.Bonus] =
                        ReadEnchantData(Enums.TBC.EnchantmentSlot.Bonus);
                    updateData.ItemData.Enchantment[EnchantmentSlot.Prop0] =
                        ReadEnchantData(Enums.TBC.EnchantmentSlot.Prop0);
                    updateData.ItemData.Enchantment[EnchantmentSlot.Prop1] =
                        ReadEnchantData(Enums.TBC.EnchantmentSlot.Prop1);
                    updateData.ItemData.Enchantment[EnchantmentSlot.Prop2] =
                        ReadEnchantData(Enums.TBC.EnchantmentSlot.Prop2);
                    updateData.ItemData.Enchantment[EnchantmentSlot.Prop3] =
                        ReadEnchantData(Enums.TBC.EnchantmentSlot.Prop3);
                    updateData.ItemData.Enchantment[EnchantmentSlot.Prop4] =
                        ReadEnchantData(Enums.TBC.EnchantmentSlot.Prop4);
                }
                else
                {
                    updateData.ItemData.Enchantment[EnchantmentSlot.Perm] =
                        ReadEnchantData(Enums.WotLK.EnchantmentSlot.Perm);
                    updateData.ItemData.Enchantment[EnchantmentSlot.Temp] =
                        ReadEnchantData(Enums.WotLK.EnchantmentSlot.Temp);
                    updateData.ItemData.Enchantment[EnchantmentSlot.Sock1] =
                        ReadEnchantData(Enums.WotLK.EnchantmentSlot.Sock1);
                    updateData.ItemData.Enchantment[EnchantmentSlot.Sock2] =
                        ReadEnchantData(Enums.WotLK.EnchantmentSlot.Sock2);
                    updateData.ItemData.Enchantment[EnchantmentSlot.Sock3] =
                        ReadEnchantData(Enums.WotLK.EnchantmentSlot.Sock3);
                    updateData.ItemData.Enchantment[EnchantmentSlot.Bonus] =
                        ReadEnchantData(Enums.WotLK.EnchantmentSlot.Bonus);
                    updateData.ItemData.Enchantment[EnchantmentSlot.Prismatic] =
                        ReadEnchantData(Enums.WotLK.EnchantmentSlot.Prismatic);
                    updateData.ItemData.Enchantment[EnchantmentSlot.Prop0] =
                        ReadEnchantData(Enums.WotLK.EnchantmentSlot.Prop0);
                    updateData.ItemData.Enchantment[EnchantmentSlot.Prop1] =
                        ReadEnchantData(Enums.WotLK.EnchantmentSlot.Prop1);
                    updateData.ItemData.Enchantment[EnchantmentSlot.Prop2] =
                        ReadEnchantData(Enums.WotLK.EnchantmentSlot.Prop2);
                    updateData.ItemData.Enchantment[EnchantmentSlot.Prop3] =
                        ReadEnchantData(Enums.WotLK.EnchantmentSlot.Prop3);
                    updateData.ItemData.Enchantment[EnchantmentSlot.Prop4] =
                        ReadEnchantData(Enums.WotLK.EnchantmentSlot.Prop4);
                }
                ...
                for (var i2 = 0; i2 < 3; i2++)
                {
                    ...
                    if (updateData.ItemData.Enchantment[slot] != null &&
                        updateData.ItemData.Enchantment[slot].ID.HasValue)
                    {
            ...
            var ITEM_FIELD_RANDOM_PROPERTIES_ID =
                LegacyVersion.GetUpdateField(ItemField.ITEM_FIELD_RANDOM_PROPERTIES_ID);
        ...
        if (objectType == ObjectType.Container)
        {
            ...
            if (CONTAINER_FIELD_SLOT_1 >= 0)
            {
                for (var i3 = 0; i3 < 36; i3++)
                {
                    ...
                    {
                        updateData.ContainerData.Slots[i3] = GetGuidValue(updates, CONTAINER_FIELD_SLOT_1 + i3 * 2)
                            .To128(GetSession().GameState);
                    }
                }
            }
        ...
        if (objectType == ObjectType.Unit || objectType == ObjectType.Player || objectType == ObjectType.ActivePlayer)
        {
            ...
            {
                updateData.UnitData.Charm =
                    GetGuidValue(updates, UnitField.UNIT_FIELD_CHARM).To128(GetSession().GameState);
            }

            var UNIT_FIELD_SUMMON = LegacyVersion.GetUpdateField(UnitField.UNIT_FIELD_SUMMON);
            if (UNIT_FIELD_SUMMON >= 0 && updateMaskArray[UNIT_FIELD_SUMMON])
            {
                updateData.UnitData.Summon =
                    GetGuidValue(updates, UnitField.UNIT_FIELD_SUMMON).To128(GetSession().GameState);
            }
            ...
            {
                updateData.UnitData.CharmedBy =
                    GetGuidValue(updates, UnitField.UNIT_FIELD_CHARMEDBY).To128(GetSession().GameState);
            }
            ...
            {
                updateData.UnitData.SummonedBy = GetGuidValue(updates, UnitField.UNIT_FIELD_SUMMONEDBY)
                    .To128(GetSession().GameState);
            }
            ...
            {
                updateData.UnitData.CreatedBy =
                    GetGuidValue(updates, UnitField.UNIT_FIELD_CREATEDBY).To128(GetSession().GameState);
            }
            ...
            {
                updateData.UnitData.Target =
                    GetGuidValue(updates, UnitField.UNIT_FIELD_TARGET).To128(GetSession().GameState);
            }
            ...
            {
                updateData.UnitData.ChannelObject = GetGuidValue(updates, UnitField.UNIT_FIELD_CHANNEL_OBJECT)
                    .To128(GetSession().GameState);
            }
            ...
            if (UNIT_FIELD_BYTES_0 >= 0 && updateMaskArray[UNIT_FIELD_BYTES_0])
            {
                ...
                {
                    GetSession().GameState
                        .StoreCreatureClass(guid.GetEntry(), (Class)updateData.UnitData.ClassId.Value);
                }
                else
                {
                    updateData.PlayerData.ArenaFaction =
                        (byte)(GameData.IsAllianceRace((Race)updateData.UnitData.RaceId.Value) ? 1u : 0u);
                }
            }
                ...
                for (var i4 = 0; i4 < LegacyVersion.GetPowersCount(); i4++)
                {
                    if (updateMaskArray[UNIT_FIELD_POWER1 + i4])
                    {
                        if (powerUpdate != null && (guid == GetSession().GameState.CurrentPlayerGuid ||
                                                    guid == GetSession().GameState.CurrentPetGuid))
                        {
                            powerUpdate.Powers.Add(new PowerUpdatePower(updates[UNIT_FIELD_POWER1 + i4].Int32Value,
                                (byte)i4));
                        }
                        ...
                        {
                            var classId = !updateData.UnitData.ClassId.HasValue
                                ? GetSession().GameState.GetUnitClass(guid.To128(GetSession().GameState))
                                : (Class)updateData.UnitData.ClassId.Value;
            ...
            if (UNIT_FIELD_MAXPOWER1 >= 0)
            {
                for (var i5 = 0; i5 < LegacyVersion.GetPowersCount(); i5++)
                {
                    ...
                    var classId2 = !updateData.UnitData.ClassId.HasValue
                        ? GetSession().GameState.GetUnitClass(guid.To128(GetSession().GameState))
                        : (Class)updateData.UnitData.ClassId.Value;
                    var powerSlot2 = !GetSession().GameState.HunterPetGuids.Contains(guid)
                        ? ClassPowerTypes.GetPowerSlotForClass(classId2, (PowerType)i5)
                        : ClassPowerTypes.GetPowerSlotForPet((PowerType)i5);
                    if (powerSlot2 >= 0)
                    {
            ...
            var UNIT_FIELD_POWER_REGEN_FLAT_MODIFIER =
                LegacyVersion.GetUpdateField(UnitField.UNIT_FIELD_POWER_REGEN_FLAT_MODIFIER);
            if (UNIT_FIELD_POWER_REGEN_FLAT_MODIFIER >= 0)
            {
                for (var iPR = 0; iPR < 7; iPR++)
                {
                    ...
                    {
                        updateData.UnitData.ModPowerRegen[iPR] =
                            updates[UNIT_FIELD_POWER_REGEN_FLAT_MODIFIER + iPR].FloatValue;
                    }
                }
            }
            ...
            if (UNIT_FIELD_FLAGS >= 0 && updateMaskArray[UNIT_FIELD_FLAGS])
            {
                ...
                if (LegacyVersion.RemovedInVersion(ClientVersionBuild.V3_0_2_9056) &&
                    !updateData.UnitData.PvpFlags.HasValue)
                {
            ...
            if (UNIT_FIELD_BASEATTACKTIME >= 0)
            {
                for (var i8 = 0; i8 < 2; i8++)
                {
                    ...
                    {
                        updateData.UnitData.AttackRoundBaseTime[i8] =
                            updates[UNIT_FIELD_BASEATTACKTIME + i8].UInt32Value;
                    }
                }
            }
            ...
            if (UNIT_FIELD_RANGEDATTACKTIME >= 0 && updateMaskArray[UNIT_FIELD_RANGEDATTACKTIME])
            {
                ...
                Log.Print(LogType.Debug,
                    $"[UnitField] RangedAttackRoundBaseTime = {updates[UNIT_FIELD_RANGEDATTACKTIME].UInt32Value}",
                    "HandleUpdateObject", "");
            }
            ...
            if (UNIT_FIELD_DISPLAYID >= 0 && updateMaskArray[UNIT_FIELD_DISPLAYID])
            {
                ...
                if (LegacyVersion.RemovedInVersion(ClientVersionBuild.V2_0_1_6180))
                {
                    updateData.UnitData.DisplayScale =
                        1f / GameData.GetUnitCompleteDisplayScale((uint)updateData.UnitData.DisplayID.Value);
                }
            }
            ...
            if (UNIT_DYNAMIC_FLAGS >= 0 && updateMaskArray[UNIT_DYNAMIC_FLAGS])
            {
                ...
                if (flags.HasFlag(UnitDynamicFlagsLegacy.Tapped) &&
                    flags.HasFlag(UnitDynamicFlagsLegacy.TappedByPlayer))
                {
            ...
            if (UNIT_CREATED_BY_SPELL >= 0 && updateMaskArray[UNIT_CREATED_BY_SPELL])
            {
                ...
                if (LegacyVersion.RemovedInVersion(ClientVersionBuild.V2_0_1_6180) && isCreate &&
                    updateData.UnitData.CreatedBy == GetSession().GameState.CurrentPlayerGuid)
                {
            ...
            if (UNIT_TRAINING_POINTS >= 0 && updateMaskArray[UNIT_TRAINING_POINTS])
            {
                ...
                updateData.UnitData.TrainingPointsTotal =
                    (ushort)((updates[UNIT_TRAINING_POINTS].UInt32Value >> 16) & 0xFFFF);
            }
            ...
            var UNIT_FIELD_RESISTANCEBUFFMODSPOSITIVE =
                LegacyVersion.GetUpdateField(UnitField.UNIT_FIELD_RESISTANCEBUFFMODSPOSITIVE);
            if (UNIT_FIELD_RESISTANCEBUFFMODSPOSITIVE < 0)
                UNIT_FIELD_RESISTANCEBUFFMODSPOSITIVE =
                    LegacyVersion.GetUpdateField(UnitField.UNIT_FIELD_RESISTANCEBUFFMODSPOSITIVE_ARMOR);
            if (UNIT_FIELD_RESISTANCEBUFFMODSPOSITIVE >= 0)
            {
                for (var i13 = 0; i13 < 7; i13++)
                {
                    ...
                    {
                        updateData.UnitData.ResistanceBuffModsPositive[i13] =
                            updates[UNIT_FIELD_RESISTANCEBUFFMODSPOSITIVE + i13].Int32Value;
                    }
                }
            }

            var UNIT_FIELD_RESISTANCEBUFFMODSNEGATIVE =
                LegacyVersion.GetUpdateField(UnitField.UNIT_FIELD_RESISTANCEBUFFMODSNEGATIVE);
            if (UNIT_FIELD_RESISTANCEBUFFMODSNEGATIVE < 0)
                UNIT_FIELD_RESISTANCEBUFFMODSNEGATIVE =
                    LegacyVersion.GetUpdateField(UnitField.UNIT_FIELD_RESISTANCEBUFFMODSNEGATIVE_ARMOR);
            if (UNIT_FIELD_RESISTANCEBUFFMODSNEGATIVE >= 0)
            {
                for (var i14 = 0; i14 < 7; i14++)
                {
                    ...
                    {
                        updateData.UnitData.ResistanceBuffModsNegative[i14] =
                            updates[UNIT_FIELD_RESISTANCEBUFFMODSNEGATIVE + i14].Int32Value;
                    }
                }
            }
            ...
            if (UNIT_FIELD_ATTACK_POWER_MODS >= 0 && updateMaskArray[UNIT_FIELD_ATTACK_POWER_MODS])
            {
                ...
                updateData.UnitData.AttackPowerModPos =
                    (updates[UNIT_FIELD_ATTACK_POWER_MODS].Int32Value >> 16) & 0xFFFF;
            }

            var UNIT_FIELD_RANGED_ATTACK_POWER_MULTIPLIER =
                LegacyVersion.GetUpdateField(UnitField.UNIT_FIELD_RANGED_ATTACK_POWER_MULTIPLIER);
            if (UNIT_FIELD_RANGED_ATTACK_POWER_MULTIPLIER >= 0 &&
                updateMaskArray[UNIT_FIELD_RANGED_ATTACK_POWER_MULTIPLIER])
            {
                updateData.UnitData.AttackPowerMultiplier =
                    updates[UNIT_FIELD_RANGED_ATTACK_POWER_MULTIPLIER].FloatValue;
            }
            ...
            if (UNIT_FIELD_POWER_COST_MODIFIER >= 0)
            {
                for (var i15 = 0; i15 < 7; i15++)
                {
                    ...
                    {
                        updateData.UnitData.PowerCostModifier[i15] =
                            updates[UNIT_FIELD_POWER_COST_MODIFIER + i15].Int32Value;
                    }
                }
            }

            var UNIT_FIELD_POWER_COST_MULTIPLIER =
                LegacyVersion.GetUpdateField(UnitField.UNIT_FIELD_POWER_COST_MULTIPLIER);
            if (UNIT_FIELD_POWER_COST_MULTIPLIER >= 0)
            {
                for (var i16 = 0; i16 < 7; i16++)
                {
                    ...
                    {
                        updateData.UnitData.PowerCostMultiplier[i16] =
                            updates[UNIT_FIELD_POWER_COST_MULTIPLIER + i16].FloatValue;
                    }
                }
            }
            ...
            var UNIT_FIELD_AURALEVELS = LegacyVersion.GetUpdateField(UnitField.UNIT_FIELD_AURALEVELS);
            var UNIT_FIELD_AURAAPPLICATIONS = LegacyVersion.GetUpdateField(UnitField.UNIT_FIELD_AURAAPPLICATIONS);
            if (UNIT_FIELD_AURA > 0 && UNIT_FIELD_AURAFLAGS > 0 && UNIT_FIELD_AURALEVELS > 0 &&
                UNIT_FIELD_AURAAPPLICATIONS > 0)
            {
                var aurasCount = LegacyVersion.GetAuraSlotsCount();
                for (byte i17 = 0; i17 < aurasCount; i17++)
                {
                    if (!updateMaskArray[UNIT_FIELD_AURA + i17] && !updateMaskArray[UNIT_FIELD_AURALEVELS + i17 / 4] &&
                        !updateMaskArray[UNIT_FIELD_AURAAPPLICATIONS + i17 / 4])
                    {
                        continue;
                    }
        ...
        if (objectType == ObjectType.Player || objectType == ObjectType.ActivePlayer)
        {
            ...
            {
                updateData.PlayerData.DuelArbiter = GetGuidValue(updates, PlayerField.PLAYER_DUEL_ARBITER)
                    .To128(GetSession().GameState);
            }
            ...
            if (PLAYER_FLAGS >= 0 && updateMaskArray[PLAYER_FLAGS])
            {
                ...
                if (updateData.Guid == GetSession().GameState.CurrentPlayerGuid)
                {
                    ...
                    {
                        Log.Print(LogType.Debug,
                            "[DeathRevive] Ghost→Alive transition detected, will recreate player object",
                            "StoreObjectUpdate", "");
            ...
            else if (updateData.Guid == GetSession().GameState.CurrentPlayerGuid &&
                     GetSession().GameState.CurrentPlayerStorage.Settings.NeedToForcePatchFlags)
            {
            ...
            if (PLAYER_GUILDID >= 0 && updateMaskArray[PLAYER_GUILDID])
            {
                ...
                updateData.UnitData.GuildGUID =
                    WowGuid128.Create(HighGuidType703.Guild, updates[PLAYER_GUILDID].UInt32Value);
            }
            ...
            if (PLAYER_VISIBLE_ITEM_1_0 >= 0)
            {
                ...
                for (var i19 = 0; i19 < 19; i19++)
                {
                    ...
                    if (updateMaskArray[itemIdIndex] || updateMaskArray[enchantIdIndex])
                    {
                        ...
                        {
                            updateData.PlayerData.VisibleItems[i19].ItemVisual =
                                (ushort)GameData.GetItemEnchantVisual(updates[enchantIdIndex].UInt32Value);
                        }
                    }
                }
            }
            ...
            if (PLAYER_VISIBLE_ITEM_1_ENTRYID >= 0)
            {
                ...
                for (var i20 = 0; i20 < 19; i20++)
                {
                    if (updateMaskArray[PLAYER_VISIBLE_ITEM_1_ENTRYID + i20 * offset2])
                    {
                        ...
                        if (i20 >= 15 && i20 <= 18)
                            Log.Print(LogType.Debug,
                                $"[VisibleItem] Slot {i20} ItemID={updateData.PlayerData.VisibleItems[i20].ItemID}",
                                "HandleUpdateObject", "");
                    }
                }
            }
            ...
            if (PLAYER_FIELD_INV_SLOT_HEAD >= 0)
            {
                for (var i21 = 0; i21 < 23; i21++)
                {
                    ...
                    {
                        updateData.ActivePlayerData.InvSlots[i21] =
                            GetGuidValue(updates, PLAYER_FIELD_INV_SLOT_HEAD + i21 * 2).To128(GetSession().GameState);
                        if (i21 >= 15 && i21 <= 18)
                            Log.Print(LogType.Debug,
                                $"[InvSlot] Slot {i21} = {updateData.ActivePlayerData.InvSlots[i21]}",
                                "HandleUpdateObject", "");
                    }
                }
            }
            ...
            if (PLAYER_FIELD_PACK_SLOT_1 >= 0)
            {
                for (var i22 = 0; i22 < 16; i22++)
                {
                    ...
                    {
                        updateData.ActivePlayerData.PackSlots[i22] =
                            GetGuidValue(updates, PLAYER_FIELD_PACK_SLOT_1 + i22 * 2).To128(GetSession().GameState);
                        Log.Print(LogType.Debug,
                            $"[InvUpdate] PackSlot[{i22}] = {updateData.ActivePlayerData.PackSlots[i22]} (modern idx {35 + i22})",
                            "HandleUpdateObject", "");
                    }
                }
            }
            ...
            if (PLAYER_FIELD_BANK_SLOT_1 >= 0)
            {
                ...
                for (var i23 = 0; i23 < bankSlots; i23++)
                {
                    ...
                    {
                        updateData.ActivePlayerData.BankSlots[i23] =
                            GetGuidValue(updates, PLAYER_FIELD_BANK_SLOT_1 + i23 * 2).To128(GetSession().GameState);
                        Log.Print(LogType.Debug,
                            $"[InvUpdate] BankSlot[{i23}] = {updateData.ActivePlayerData.BankSlots[i23]} (modern idx {59 + i23})",
                            "HandleUpdateObject", "");
                    }
                }
            }
            ...
            if (PLAYER_FIELD_BANKBAG_SLOT_1 >= 0)
            {
                ...
                for (var i24 = 0; i24 < bankBagSlots; i24++)
                {
                    ...
                    {
                        updateData.ActivePlayerData.BankBagSlots[i24] =
                            GetGuidValue(updates, PLAYER_FIELD_BANKBAG_SLOT_1 + i24 * 2).To128(GetSession().GameState);
                    }
                }
            }

            var PLAYER_FIELD_VENDORBUYBACK_SLOT_1 =
                LegacyVersion.GetUpdateField(PlayerField.PLAYER_FIELD_VENDORBUYBACK_SLOT_1);
            if (PLAYER_FIELD_VENDORBUYBACK_SLOT_1 >= 0)
            {
                for (var i25 = 0; i25 < 12; i25++)
                {
                    ...
                    {
                        updateData.ActivePlayerData.BuyBackSlots[i25] =
                            GetGuidValue(updates, PLAYER_FIELD_VENDORBUYBACK_SLOT_1 + i25 * 2)
                                .To128(GetSession().GameState);
                    }
                }
            }
            ...
            if (PLAYER_FIELD_KEYRING_SLOT_1 >= 0)
            {
                for (var i26 = 0; i26 < 32; i26++)
                {
                    ...
                    {
                        updateData.ActivePlayerData.KeyringSlots[i26] =
                            GetGuidValue(updates, PLAYER_FIELD_KEYRING_SLOT_1 + i26 * 2).To128(GetSession().GameState);
                    }
                }
            }
            ...
            if (skin.HasValue && face.HasValue && hairStyle.HasValue && hairColor.HasValue && facialHair.HasValue)
            {
                ...
                if ((raceId == Race.None || sexId == Gender.None) && GetSession().GameState.CachedPlayers
                        .TryGetValue(guid.To128(GetSession().GameState), out var cache))
                {
                    raceId = cache.RaceId;
                    sexId = cache.SexId;
                }
                ...
                {
                    var customizations = CharacterCustomizations.ConvertLegacyCustomizationsToModern(raceId, sexId,
                        skin.Value, face.Value, hairStyle.Value, hairColor.Value, facialHair.Value);
            ...
            {
                updateData.ActivePlayerData.FarsightObject =
                    GetGuidValue(updates, PlayerField.PLAYER_FARSIGHT).To128(GetSession().GameState);
            }
            ...
            {
                updateData.ActivePlayerData.ComboTarget = GetGuidValue(updates, PlayerField.PLAYER_FIELD_COMBO_TARGET)
                    .To128(GetSession().GameState);
            }
            ...
            if (PLAYER_FIELD_KNOWN_TITLES >= 0)
            {
                ...
                for (var i28 = 0; i28 < count; i28++)
                {
                    ...
                    {
                        updateData.ActivePlayerData.KnownTitles[i28] =
                            updates[PLAYER_FIELD_KNOWN_TITLES + i28].UInt32Value;
                    }
                }
            }
            ...
            if (PLAYER_SKILL_INFO_1_1 >= 0)
            {
                for (var i29 = 0; i29 < 128; i29++)
                {
                    ...
                    {
                        updateData.ActivePlayerData.Skill.SkillLineID[i29] =
                            (ushort)(updates[idIndex].UInt32Value & 0xFFFF);
                        updateData.ActivePlayerData.Skill.SkillStep[i29] =
                            (ushort)((updates[idIndex].UInt32Value >> 16) & 0xFFFF);
                    }

                    var valueIndex = idIndex + 1;
                    if (updateMaskArray[valueIndex])
                    {
                        updateData.ActivePlayerData.Skill.SkillRank[i29] =
                            (ushort)(updates[valueIndex].UInt32Value & 0xFFFF);
                        updateData.ActivePlayerData.Skill.SkillMaxRank[i29] =
                            (ushort)((updates[valueIndex].UInt32Value >> 16) & 0xFFFF);
                    }

                    var bonusIndex = valueIndex + 1;
                    if (updateMaskArray[bonusIndex])
                    {
                        updateData.ActivePlayerData.Skill.SkillTempBonus[i29] =
                            (short)(updates[bonusIndex].Int32Value & 0xFFFF);
                        updateData.ActivePlayerData.Skill.SkillPermBonus[i29] =
                            (ushort)((updates[bonusIndex].UInt32Value >> 16) & 0xFFFF);
                    }
                }
            }
            ...
            var PLAYER_OFFHAND_CRIT_PERCENTAGE =
                LegacyVersion.GetUpdateField(PlayerField.PLAYER_OFFHAND_CRIT_PERCENTAGE);
            ...
            if (PLAYER_SPELL_CRIT_PERCENTAGE1 >= 0)
            {
                for (var i30 = 0; i30 < 7; i30++)
                {
                    ...
                    {
                        updateData.ActivePlayerData.SpellCritPercentage[i30] =
                            updates[PLAYER_SPELL_CRIT_PERCENTAGE1 + i30].FloatValue;
                    }
                }
            }
            ...
            if (PLAYER_EXPLORED_ZONES_1 >= 0)
            {
                ...
                for (var i31 = 0; i31 < maxZones; i31++)
                {
                    if (updateMaskArray[PLAYER_EXPLORED_ZONES_1 + i31])
                    {
                        if ((i31 & 1) != 0)
                        {
                            var oldValue = updateData.ActivePlayerData.ExploredZones[i31 / 2].HasValue
                                ? updateData.ActivePlayerData.ExploredZones[i31 / 2].Value
                                : 0;
                            updateData.ActivePlayerData.ExploredZones[i31 / 2] = oldValue |
                                ((ulong)updates[PLAYER_EXPLORED_ZONES_1 + i31].UInt32Value << 32);
                        }
                        else
                        {
                            updateData.ActivePlayerData.ExploredZones[i31 / 2] =
                                updates[PLAYER_EXPLORED_ZONES_1 + i31].UInt32Value;
                        }
                    }
                }
            }
            ...
            var PLAYER_FIELD_RESISTANCEBUFFMODSPOSITIVE =
                LegacyVersion.GetUpdateField(PlayerField.PLAYER_FIELD_RESISTANCEBUFFMODSPOSITIVE);
            if (PLAYER_FIELD_RESISTANCEBUFFMODSPOSITIVE >= 0)
            {
                for (var i34 = 0; i34 < 7; i34++)
                {
                    ...
                    {
                        updateData.UnitData.ResistanceBuffModsPositive[i34] =
                            updates[PLAYER_FIELD_RESISTANCEBUFFMODSPOSITIVE + i34].Int32Value;
                    }
                }
            }

            var PLAYER_FIELD_RESISTANCEBUFFMODSNEGATIVE =
                LegacyVersion.GetUpdateField(PlayerField.PLAYER_FIELD_RESISTANCEBUFFMODSNEGATIVE);
            if (PLAYER_FIELD_RESISTANCEBUFFMODSNEGATIVE >= 0)
            {
                for (var i35 = 0; i35 < 7; i35++)
                {
                    ...
                    {
                        updateData.UnitData.ResistanceBuffModsNegative[i35] =
                            updates[PLAYER_FIELD_RESISTANCEBUFFMODSNEGATIVE + i35].Int32Value;
                    }
                }
            }

            var PLAYER_FIELD_MOD_DAMAGE_DONE_POS =
                LegacyVersion.GetUpdateField(PlayerField.PLAYER_FIELD_MOD_DAMAGE_DONE_POS);
            if (PLAYER_FIELD_MOD_DAMAGE_DONE_POS >= 0)
            {
                for (var i36 = 0; i36 < 7; i36++)
                {
                    ...
                    {
                        updateData.ActivePlayerData.ModDamageDonePos[i36] =
                            updates[PLAYER_FIELD_MOD_DAMAGE_DONE_POS + i36].Int32Value;
                    }
                }
            }

            var PLAYER_FIELD_MOD_DAMAGE_DONE_NEG =
                LegacyVersion.GetUpdateField(PlayerField.PLAYER_FIELD_MOD_DAMAGE_DONE_NEG);
            if (PLAYER_FIELD_MOD_DAMAGE_DONE_NEG >= 0)
            {
                for (var i37 = 0; i37 < 7; i37++)
                {
                    ...
                    {
                        updateData.ActivePlayerData.ModDamageDoneNeg[i37] =
                            updates[PLAYER_FIELD_MOD_DAMAGE_DONE_NEG + i37].Int32Value;
                    }
                }
            }

            var PLAYER_FIELD_MOD_DAMAGE_DONE_PCT =
                LegacyVersion.GetUpdateField(PlayerField.PLAYER_FIELD_MOD_DAMAGE_DONE_PCT);
            if (PLAYER_FIELD_MOD_DAMAGE_DONE_PCT >= 0)
            {
                for (var i38 = 0; i38 < 7; i38++)
                {
                    ...
                    {
                        updateData.ActivePlayerData.ModDamageDonePercent[i38] =
                            updates[PLAYER_FIELD_MOD_DAMAGE_DONE_PCT + i38].FloatValue;
                    }
                }
            }

            var PLAYER_FIELD_MOD_HEALING_DONE_POS =
                LegacyVersion.GetUpdateField(PlayerField.PLAYER_FIELD_MOD_HEALING_DONE_POS);
            ...
            var PLAYER_FIELD_MOD_TARGET_RESISTANCE =
                LegacyVersion.GetUpdateField(PlayerField.PLAYER_FIELD_MOD_TARGET_RESISTANCE);
            if (PLAYER_FIELD_MOD_TARGET_RESISTANCE >= 0 && updateMaskArray[PLAYER_FIELD_MOD_TARGET_RESISTANCE])
            {
                updateData.ActivePlayerData.ModTargetResistance =
                    updates[PLAYER_FIELD_MOD_TARGET_RESISTANCE].Int32Value;
            }

            var PLAYER_FIELD_MOD_TARGET_PHYSICAL_RESISTANCE =
                LegacyVersion.GetUpdateField(PlayerField.PLAYER_FIELD_MOD_TARGET_PHYSICAL_RESISTANCE);
            if (PLAYER_FIELD_MOD_TARGET_PHYSICAL_RESISTANCE >= 0 &&
                updateMaskArray[PLAYER_FIELD_MOD_TARGET_PHYSICAL_RESISTANCE])
            {
                updateData.ActivePlayerData.ModTargetPhysicalResistance =
                    updates[PLAYER_FIELD_MOD_TARGET_PHYSICAL_RESISTANCE].Int32Value;
            }
            ...
            if (PLAYER_FIELD_BYTES >= 0 && updateMaskArray[PLAYER_FIELD_BYTES])
            {
                ...
                if (LegacyVersion.RemovedInVersion(ClientVersionBuild.V2_0_1_6180))
                {
                    ...
                    classId3 = !updateData.UnitData.ClassId.HasValue
                        ? GetSession().GameState.GetUnitClass(guid.To128(GetSession().GameState))
                        : (Class)updateData.UnitData.ClassId.Value;
                ...
                {
                    updateData.ActivePlayerData.GrantableLevels =
                        (byte)((updates[PLAYER_FIELD_BYTES].UInt32Value >> 8) & 0xFF);
                }

                updateData.ActivePlayerData.MultiActionBars =
                    (byte)((updates[PLAYER_FIELD_BYTES].UInt32Value >> 16) & 0xFF);
                updateData.ActivePlayerData.LifetimeMaxRank =
                    (byte)((updates[PLAYER_FIELD_BYTES].UInt32Value >> 24) & 0xFF);
            }
            ...
            if (PLAYER_FIELD_BUYBACK_PRICE_1 >= 0)
            {
                for (var i39 = 0; i39 < 12; i39++)
                {
                    ...
                    {
                        updateData.ActivePlayerData.BuybackPrice[i39] =
                            updates[PLAYER_FIELD_BUYBACK_PRICE_1 + i39].UInt32Value;
                    }
                }
            }

            var PLAYER_FIELD_BUYBACK_TIMESTAMP_1 =
                LegacyVersion.GetUpdateField(PlayerField.PLAYER_FIELD_BUYBACK_TIMESTAMP_1);
            if (PLAYER_FIELD_BUYBACK_TIMESTAMP_1 >= 0)
            {
                for (var i40 = 0; i40 < 12; i40++)
                {
                    ...
                    {
                        updateData.ActivePlayerData.BuybackTimestamp[i40] =
                            updates[PLAYER_FIELD_BUYBACK_TIMESTAMP_1 + i40].UInt32Value;
                    }
                }
            }
            ...
            {
                updateData.ActivePlayerData.TodayHonorableKills =
                    (ushort)(updates[PLAYER_FIELD_SESSION_KILLS].UInt32Value & 0xFFFF);
                updateData.ActivePlayerData.TodayDishonorableKills =
                    (ushort)((updates[PLAYER_FIELD_SESSION_KILLS].UInt32Value >> 16) & 0xFFFF);
            }
            ...
            {
                updateData.ActivePlayerData.TodayHonorableKills =
                    (ushort)(updates[PLAYER_FIELD_KILLS].UInt32Value & 0xFFFF);
                updateData.ActivePlayerData.YesterdayHonorableKills =
                    (ushort)((updates[PLAYER_FIELD_KILLS].UInt32Value >> 16) & 0xFFFF);
            }
            ...
            {
                updateData.ActivePlayerData.YesterdayHonorableKills =
                    (ushort)(updates[PLAYER_FIELD_YESTERDAY_KILLS].UInt32Value & 0xFFFF);
                updateData.ActivePlayerData.YesterdayDishonorableKills =
                    (ushort)((updates[PLAYER_FIELD_YESTERDAY_KILLS].UInt32Value >> 16) & 0xFFFF);
            }
            ...
            {
                updateData.ActivePlayerData.LastWeekHonorableKills =
                    (ushort)(updates[PLAYER_FIELD_LAST_WEEK_KILLS].UInt32Value & 0xFFFF);
                updateData.ActivePlayerData.LastWeekDishonorableKills =
                    (ushort)((updates[PLAYER_FIELD_LAST_WEEK_KILLS].UInt32Value >> 16) & 0xFFFF);
            }
            ...
            {
                updateData.ActivePlayerData.ThisWeekHonorableKills =
                    (ushort)(updates[PLAYER_FIELD_THIS_WEEK_KILLS].UInt32Value & 0xFFFF);
                updateData.ActivePlayerData.ThisWeekDishonorableKills =
                    (ushort)((updates[PLAYER_FIELD_THIS_WEEK_KILLS].UInt32Value >> 16) & 0xFFFF);
            }

            var PLAYER_FIELD_THIS_WEEK_CONTRIBUTION =
                LegacyVersion.GetUpdateField(PlayerField.PLAYER_FIELD_THIS_WEEK_CONTRIBUTION);
            if (PLAYER_FIELD_THIS_WEEK_CONTRIBUTION < 0)
            {
                PLAYER_FIELD_THIS_WEEK_CONTRIBUTION =
                    LegacyVersion.GetUpdateField(PlayerField.PLAYER_FIELD_TODAY_CONTRIBUTION);
            }

            if (PLAYER_FIELD_THIS_WEEK_CONTRIBUTION >= 0 && updateMaskArray[PLAYER_FIELD_THIS_WEEK_CONTRIBUTION])
            {
                updateData.ActivePlayerData.ThisWeekContribution =
                    updates[PLAYER_FIELD_THIS_WEEK_CONTRIBUTION].UInt32Value;
            }

            var PLAYER_FIELD_LIFETIME_HONORABLE_KILLS =
                LegacyVersion.GetUpdateField(PlayerField.PLAYER_FIELD_LIFETIME_HONORABLE_KILLS);
            if (PLAYER_FIELD_LIFETIME_HONORABLE_KILLS >= 0 && updateMaskArray[PLAYER_FIELD_LIFETIME_HONORABLE_KILLS])
            {
                updateData.ActivePlayerData.LifetimeHonorableKills =
                    updates[PLAYER_FIELD_LIFETIME_HONORABLE_KILLS].UInt32Value;
            }

            var PLAYER_FIELD_LIFETIME_DISHONORABLE_KILLS =
                LegacyVersion.GetUpdateField(PlayerField.PLAYER_FIELD_LIFETIME_DISHONORABLE_KILLS);
            if (PLAYER_FIELD_LIFETIME_DISHONORABLE_KILLS >= 0 &&
                updateMaskArray[PLAYER_FIELD_LIFETIME_DISHONORABLE_KILLS])
            {
                updateData.ActivePlayerData.LifetimeDishonorableKills =
                    updates[PLAYER_FIELD_LIFETIME_DISHONORABLE_KILLS].UInt32Value;
            }

            var PLAYER_FIELD_YESTERDAY_CONTRIBUTION =
                LegacyVersion.GetUpdateField(PlayerField.PLAYER_FIELD_YESTERDAY_CONTRIBUTION);
            if (PLAYER_FIELD_YESTERDAY_CONTRIBUTION >= 0 && updateMaskArray[PLAYER_FIELD_YESTERDAY_CONTRIBUTION])
            {
                updateData.ActivePlayerData.YesterdayContribution =
                    updates[PLAYER_FIELD_YESTERDAY_CONTRIBUTION].UInt32Value;
            }

            var PLAYER_FIELD_LAST_WEEK_CONTRIBUTION =
                LegacyVersion.GetUpdateField(PlayerField.PLAYER_FIELD_LAST_WEEK_CONTRIBUTION);
            if (PLAYER_FIELD_LAST_WEEK_CONTRIBUTION >= 0 && updateMaskArray[PLAYER_FIELD_LAST_WEEK_CONTRIBUTION])
            {
                updateData.ActivePlayerData.LastWeekContribution =
                    updates[PLAYER_FIELD_LAST_WEEK_CONTRIBUTION].UInt32Value;
            }
            ...
            var PLAYER_FIELD_WATCHED_FACTION_INDEX =
                LegacyVersion.GetUpdateField(PlayerField.PLAYER_FIELD_WATCHED_FACTION_INDEX);
            if (PLAYER_FIELD_WATCHED_FACTION_INDEX >= 0 && updateMaskArray[PLAYER_FIELD_WATCHED_FACTION_INDEX])
            {
                updateData.ActivePlayerData.WatchedFactionIndex =
                    updates[PLAYER_FIELD_WATCHED_FACTION_INDEX].Int32Value;
            }
            ...
            if (PLAYER_FIELD_COMBAT_RATING_1 >= 0)
            {
                for (var i41 = 0; i41 < 20; i41++)
                {
                    ...
                    {
                        updateData.ActivePlayerData.CombatRatings[i41] =
                            updates[PLAYER_FIELD_COMBAT_RATING_1 + i41].Int32Value;
                    }
                }
            }

            var PLAYER_FIELD_ARENA_TEAM_INFO_1_1 =
                LegacyVersion.GetUpdateField(PlayerField.PLAYER_FIELD_ARENA_TEAM_INFO_1_1);
            if (PLAYER_FIELD_ARENA_TEAM_INFO_1_1 >= 0)
            {
                ...
                for (var i42 = 0; i42 < 3; i42++)
                {
                    ...
                    {
                        var teamId = GetSession().GameState.CurrentArenaTeamIds[i42] =
                            updates[startOffset + teamIdOffset].UInt32Value;
                        if (teamId != 0)
                        {
                    ...
                    if (updateMaskArray[startOffset + teamGamesWeekOffset])
                    {
                        ...
                        updateData.ActivePlayerData.PvpInfo[i42].WeeklyPlayed =
                            updates[startOffset + teamGamesWeekOffset].UInt32Value;
                    }
                    ...
                    if (updateMaskArray[startOffset + teamGamesSeasonOffset])
                    {
                        ...
                        updateData.ActivePlayerData.PvpInfo[i42].SeasonPlayed =
                            updates[startOffset + teamGamesSeasonOffset].UInt32Value;
                    }
                    ...
                    if (updateMaskArray[startOffset + teamWinsSeasonOffset])
                    {
                        ...
                        updateData.ActivePlayerData.PvpInfo[i42].SeasonWon =
                            updates[startOffset + teamWinsSeasonOffset].UInt32Value;
                    }
                    ...
                    if (updateMaskArray[startOffset + teamPersonalRatingOffset])
                    {
                        ...
                        updateData.ActivePlayerData.PvpInfo[i42].Rating =
                            updates[startOffset + teamPersonalRatingOffset].UInt32Value;
                    }
                }
            }
            ...
            if (PLAYER_FIELD_DAILY_QUESTS_1 >= 0 && guid == GetSession().GameState.CurrentPlayerGuid)
            {
                for (var i43 = 0; i43 < 25; i43++)
                {
                    ...
                    {
                        GetSession().GameState.SetDailyQuestSlot((uint)i43,
                            updates[PLAYER_FIELD_DAILY_QUESTS_1 + i43].UInt32Value);
        ...
        if (objectType == ObjectType.GameObject)
        {
            ...
            {
                updateData.GameObjectData.CreatedBy = GetGuidValue(updates, GameObjectField.GAMEOBJECT_FIELD_CREATED_BY)
                    .To128(GetSession().GameState);
            }
        ...
        if (objectType == ObjectType.DynamicObject)
        {
            ...
            {
                updateData.DynamicObjectData.Caster = GetGuidValue(updates, DynamicObjectField.DYNAMICOBJECT_CASTER)
                    .To128(GetSession().GameState);
            }
            ...
            if (DYNAMICOBJECT_SPELLID >= 0 && updateMaskArray[DYNAMICOBJECT_SPELLID])
            {
                ...
                updateData.DynamicObjectData.SpellXSpellVisualID =
                    (int)GameData.GetSpellVisual((uint)updateData.DynamicObjectData.SpellID.Value);
            }
        ...
        {
            updateData.CorpseData.Owner =
                GetGuidValue(updates, CorpseField.CORPSE_FIELD_OWNER).To128(GetSession().GameState);
        }
        ...
        if (CORPSE_FIELD_BYTES_1 >= 0 && updateMaskArray[CORPSE_FIELD_BYTES_1])
        {
            ...
            if (CORPSE_FIELD_BYTES_2 >= 0 && updateMaskArray[CORPSE_FIELD_BYTES_2])
            {
                ...
                var customizations2 = CharacterCustomizations.ConvertLegacyCustomizationsToModern(
                    (Race)updateData.CorpseData.RaceId.Value, (Gender)updateData.CorpseData.SexId.Value, skin2, face2,
                    hairStyle2, hairColor2, facialHair2);
        ...
        {
            updateData.CorpseData.GuildGUID =
                WowGuid128.Create(HighGuidType703.Guild, updates[CORPSE_FIELD_GUILD].UInt32Value);
        }
    ...
    private void HandleInitWorldStates(WorldPacket packet)
    {
        ...
        states.AreaID = LegacyVersion.AddedInVersion(ClientVersionBuild.V2_1_0_6692)
            ? packet.ReadUInt32()
            : states.ZoneID;
    ...
    public bool ConnectToWorldServer(Realm realm, GlobalSessionData globalSession)
    {
        ...
        try
        {
            ...
            Log.Print(LogType.Network,
                $"World Server address {realm.ExternalAddress}:{realm.Port} resolved as {ip}:{realm.Port}",
                "WorldClient.cs");
    ...
    private async Task<bool> ReceiveBufferFully(ArraySegment<byte> bufferToFill)
    {
        ...
        {
            if (bufferToFill.Array != null)
            {
                var tmpArrayBuffer = new ArraySegment<byte>(bufferToFill.Array, alreadyReceived + bufferToFill.Offset,
                    bufferToFill.Count - alreadyReceived);
    ...
    private async Task ReceiveLoop()
    {
        try
        {
            while (true)
            {
                ...
                {
                    Log.PrintNet(LogType.Debug, LogNetDir.S2P,
                        $"Decoded header: size={packetSize}, opcode={header.Opcode} (0x{header.Opcode:X4}), crypt={(_worldCrypt != null ? "ON" : "OFF")}",
                        "WorldClient.cs");
                }
        ...
        catch (Exception ex)
        {
            ...
            Log.PrintNet(LogType.Error, LogNetDir.S2P,
                "Packet Read Error: " + e.Message + Environment.NewLine + e.StackTrace, "WorldClient.cs");
            if (!_isSuccessful.HasValue)
            {
                _isSuccessful = false;
                return;
    ...
    private void SendPacket(WorldPacket packet)
    {
        ...
        try
        {
            ...
            Log.PrintNet(LogType.Debug, LogNetDir.P2S,
                $"Sending opcode {LegacyVersion.GetUniversalOpcode(header.Opcode)} ({header.Opcode}) with size {header.Size}.",
                "WorldClient.cs");
            ...
            Log.PrintNet(LogType.Debug, LogNetDir.P2S,
                $"Total bytes on wire: {finalData.Length}, first 16: {BitConverter.ToString(finalData, 0, Math.Min(finalData.Length, 16))}",
                "WorldClient.cs");
    ...
    private void SendPacketToClientDirect(ServerPacket packet)
    {
        ...
        {
            Log.PrintNet(LogType.Warn, LogNetDir.P2C,
                $"Dropping {packet.GetUniversalOpcode()} - session/gamestate not ready", "WorldClient.cs");
            return;
        }
        ...
        if (packet.GetConnection() == ConnectionType.Realm)
        {
            ...
            {
                Log.PrintNet(LogType.Warn, LogNetDir.P2C,
                    $"Queuing {packet.GetUniversalOpcode()} - RealmSocket not ready yet", "WorldClient.cs");
                lock (pendingPackets)
                {
        ...
        if (GetSession().InstanceSocket == null && !GetSession().GameState.IsConnectedToInstance)
        {
            lock (pendingPackets)
            {
                if (GetSession().InstanceSocket == null && !GetSession().GameState.IsConnectedToInstance)
                {
                    pendingPackets.Enqueue(packet);
                    Log.PrintNet(LogType.Warn, LogNetDir.P2C,
                        $"Can't send opcode {packet.GetUniversalOpcode()} ({packet.GetOpcode()}) before entering world! Queue (Initial Check)",
                        "WorldClient.cs");
                    return;
                }
            }
        ...
        while (GetSession().InstanceSocket == null && GetSession().GameState.IsConnectedToInstance)
        {
            Log.PrintNet(LogType.Network, LogNetDir.P2C,
                $"Waiting to send {packet.GetUniversalOpcode()} ({packet.GetOpcode()}).", "WorldClient.cs");
            Thread.Sleep(200);
        ...
        if (GetSession().InstanceSocket == null)
        {
            lock (pendingPackets)
            {
                pendingPackets.Enqueue(packet);
                Log.PrintNet(LogType.Warn, LogNetDir.P2C,
                    $"Can't send opcode {packet.GetUniversalOpcode()} ({packet.GetOpcode()}) before entering world! Queue (State: {GetSession().GameState.IsConnectedToInstance})",
                    "WorldClient.cs");
                return;
            }
        }
    ...
    private void HandlePacket(WorldPacket packet)
    {
        ...
        switch (universalOpcode)
        {
            ...
            default:
                ...
                Log.PrintNet(LogType.Warn, LogNetDir.S2P,
                    $"No handler for opcode {universalOpcode} ({packet.GetOpcode()}) (Got unknown packet from WorldServer)",
                    "WorldClient.cs");
    ...
    {
        var zero = 0u;
        var authResponse = HashAlgorithm.SHA1.Hash(Encoding.ASCII.GetBytes(_username.ToUpper()),
            BitConverter.GetBytes(zero), BitConverter.GetBytes(clientSeed), BitConverter.GetBytes(serverSeed),
    ...
    public void InitializePacketHandlers()
    {
        ...
        foreach (var methodInfo in methods)
        {
            foreach (var msgAttr in methodInfo.GetCustomAttributes<PacketHandlerAttribute>())
            {
                ...
                {
                    Log.Print(LogType.Error,
                        $"Tried to override OpcodeHandler of {_packetHandlers[msgAttr.Opcode]} with {methodInfo.Name} (Opcode {msgAttr.Opcode})",
                        "WorldClient.cs");
                }
                else
                {
                    ...
                    var del = (Action<WorldPacket>)Delegate.CreateDelegate(typeof(Action<WorldPacket>), this,
                        methodInfo);
