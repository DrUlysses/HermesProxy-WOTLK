...
public class UpdateFieldsArray
{
	...
	public void SetUpdateField<T>(object index, T value, byte offset = 0) where T : new()
	{
		if (value is byte byteValue)
		{
			...
			else if ((byte)(m_updateValues[(int)index].UnsignedValue >> offset * 8) != byteValue)
			{
		...
		else if (value is ushort ushortValue)
		{
			...
			else if ((ushort)(GetUpdateField<uint>(index) >> offset * 16) != ushortValue)
			{
					...
					if (!(val is uint))
					{
						if (!(val is float))
						{
							if (!(val is ulong))
							{
								...
								result = (T)Convert.ChangeType(new WowGuid128(GetUpdateField<ulong>((int)index + 2), GetUpdateField<ulong>(index)), typeof(T));
							}
							else
							{
								result = (T)Convert.ChangeType(((ulong)m_updateValues[(int)index + 1].UnsignedValue << 32) | m_updateValues[(int)index].UnsignedValue, typeof(T));
							}
						}
						else
						{
							result = (T)Convert.ChangeType(m_updateValues[(int)index].FloatValue, typeof(T));
						}
					}
					else
					{
						result = (T)Convert.ChangeType(m_updateValues[(int)index].UnsignedValue, typeof(T));
					}
				}
				else
				{
					result = (T)Convert.ChangeType(m_updateValues[(int)index].SignedValue, typeof(T));
				}
			}
			else
			{
				result = (T)Convert.ChangeType((ushort)(m_updateValues[(int)index].UnsignedValue >> offset * 16) & 0xFFFF, typeof(T));
			}
		}
		else
		{
			result = (T)Convert.ChangeType((byte)(m_updateValues[(int)index].UnsignedValue >> offset * 8) & 0xFF, typeof(T));
		}
		if (1 == 0)
		{
		}
	...
	{
		uint oldValue = m_updateValues[(int)index].UnsignedValue;
	...
	{
		ulong oldValue = GetUpdateField<ulong>(index);
