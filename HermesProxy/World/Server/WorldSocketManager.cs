...
public class WorldSocketManager : SocketManager<WorldSocket>
{
	...
	public override bool StartNetwork(string bindIp, int realmPort, int threadCount = 1)
	{
		...
		{
			Log.Print(LogType.Error, "StartNetwork failed to start instance AsyncAcceptor", "WorldSocketManager.cs");
			return false;
		}
	...
	{
		Log.Print(LogType.Network, "Instance socket open.", "WorldSocketManager.cs");
		try
		{
		...
		{
			Log.Print(LogType.Error, ex.ToString(), "WorldSocketManager.cs");
			return;
		}
