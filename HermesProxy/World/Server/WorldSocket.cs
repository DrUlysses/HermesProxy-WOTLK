...
public class WorldSocket : SocketBase, BnetServices.INetwork
{
    ...
    {
        CharacterCustomizations.ConvertModernCustomizationsToLegacy(alter.Customizations, out var skin, out var face,
            out var hairStyle, out var hairColor, out var facialhair);
    ...
    private void HandleCreateCharacter(CreateCharacter charCreate)
    {
        ...
        CharacterCustomizations.ConvertModernCustomizationsToLegacy(charCreate.CreateInfo.Customizations, out var skin,
            out var face, out var hairStyle, out var hairColor, out var facialhair);
    ...
    private void HandleNameQueryRequest(QueryPlayerName queryPlayerName)
    {
        ...
        SendPacketToServer(packet,
            !GetSession().GameState.IsInWorld ? Opcode.SMSG_LOGIN_VERIFY_WORLD : Opcode.MSG_NULL_ACTION);
    }
    ...
    private void HandlePlayerLogin(PlayerLogin playerLogin)
    {
        ...
        GetSession().AccountMetaDataMgr
            .SaveLastSelectedCharacter(realm.Name, selectedChar.Name, playerLogin.Guid.Low, Time.UnixTime);
        ...
        GetSession().GameState.CurrentPlayerInfo =
            GetSession().GameState.OwnCharacters.Single(x => x.CharacterGuid == playerLogin.Guid);
    ...
    private void HandleChatMessageAFK(ChatMessageAFK afk)
    {
        ...
        if (toBeSentTextParts.Count >= 1)
        {
            if (LegacyVersion.AddedInVersion(ClientVersionBuild.V2_0_1_6180))
            {
                GetSession().WorldClient
                    .SendMessageChatWotLK(ChatMessageTypeWotLK.Afk, 0u, toBeSentTextParts[0], "", "");
            }
            else
            {
                GetSession().WorldClient
                    .SendMessageChatVanilla(ChatMessageTypeVanilla.Afk, 0u, toBeSentTextParts[0], "", "");
            }
        }
    }
    ...
    private void HandleChatMessageDND(ChatMessageDND dnd)
    {
        ...
        if (toBeSentTextParts.Count >= 1)
        {
            ...
            {
                GetSession().WorldClient
                    .SendMessageChatWotLK(ChatMessageTypeWotLK.Dnd, 0u, toBeSentTextParts[0], "", "");
            }
            else
            {
                GetSession().WorldClient
                    .SendMessageChatVanilla(ChatMessageTypeVanilla.Dnd, 0u, toBeSentTextParts[0], "", "");
            }
        }
    }
    ...
    private void HandleChatMessageChannel(ChatMessageChannel channel)
    {
        ...
        foreach (var text in toBeSentTextParts)
        {
            if (LegacyVersion.AddedInVersion(ClientVersionBuild.V2_0_1_6180))
            {
                GetSession().WorldClient.SendMessageChatWotLK(ChatMessageTypeWotLK.Channel, channel.Language, text,
                    channel.Target, "");
            }
            else
            {
                GetSession().WorldClient.SendMessageChatVanilla(ChatMessageTypeVanilla.Channel, channel.Language, text,
                    channel.Target, "");
            }
        }
    }
    ...
    private void HandleChatMessageWhisper(ChatMessageWhisper whisper)
    {
        ...
        foreach (var text in toBeSentTextParts)
        {
            if (LegacyVersion.AddedInVersion(ClientVersionBuild.V2_0_1_6180))
            {
                GetSession().WorldClient.SendMessageChatWotLK(ChatMessageTypeWotLK.Whisper, whisper.Language, text, "",
                    whisper.Target);
            }
            else
            {
                GetSession().WorldClient.SendMessageChatVanilla(ChatMessageTypeVanilla.Whisper, whisper.Language, text,
                    "", whisper.Target);
            }
        }
    }
    ...
    private void HandleChatMessageEmote(ChatMessageEmote emote)
    {
        ...
        if (toBeSentTextParts.Count >= 1)
        {
            ...
            {
                GetSession().WorldClient
                    .SendMessageChatWotLK(ChatMessageTypeWotLK.Emote, 0u, toBeSentTextParts[0], "", "");
            }
            else
            {
                GetSession().WorldClient
                    .SendMessageChatVanilla(ChatMessageTypeVanilla.Emote, 0u, toBeSentTextParts[0], "", "");
            }
        }
    }
    ...
    private void HandleChatMessage(ChatMessage packet)
    {
        ...
        switch (packet.GetUniversalOpcode())
        {
            ...
            case Opcode.CMSG_CHAT_MESSAGE_INSTANCE_CHAT:
                type = !GetSession().GameState.IsInBattleground()
                    ? ChatMessageTypeModern.Party
                    : ChatMessageTypeModern.Battleground;
                break;
            default:
    ...
    private void HandleAddonMessage(ChatAddonMessage packet)
    {
        ...
        {
            var chatMsg2 =
                (ChatMessageTypeVanilla)Enum.Parse(typeof(ChatMessageTypeVanilla), packet.Params.Type.ToString());
    ...
    private void HandleAddonMessageTargeted(ChatAddonMessageTargeted packet)
    {
        var language = uint.MaxValue;
        var text = packet.Params.Prefix + "\t" + packet.Params.Text;
        var channelName = packet.ChannelGuid.IsEmpty()
            ? ""
    ...
    private void HandleGameObjUse(GameObjUse use)
    {
        ...
        Log.Print(LogType.Debug,
            $"[GameObjUse] Modern GUID={use.Guid} -> Legacy GUID={guid64} raw=0x{guid64.GetLowValue():X16}");
    ...
    private void HandleGameObjUse(GameObjReportUse use)
    {
        ...
        var guid64 = use.Guid.To64();
        Log.Print(LogType.Debug,
            $"[GameObjReportUse] Modern={use.Guid} Legacy={guid64} Entry={guid64.GetEntry()} Low={guid64.GetLowValue():X16}",
            "HandleGameObjReportUse", "");
    ...
    {
        var packet =
            new WorldPacket(note.IsPublic ? Opcode.CMSG_GUILD_SET_PUBLIC_NOTE : Opcode.CMSG_GUILD_SET_OFFICER_NOTE);
    ...
    {
        GetSession().GameState.CurrentPlayerStorage.Settings
            .SetAutoBlockGuildInvites(packet.GuildInvitesShouldGetBlocked);
        var updateData =
    ...
    private void HandleSellItem(SellItem item)
    {
        ...
        Log.Print(LogType.Debug,
            $"[SellItem] Item128={item.ItemGUID} → Item64={itemGuid64} Vendor128={item.VendorGUID} → Vendor64={vendorGuid64}",
            "");
    ...
    private void HandleSplitItem(SplitItem item)
    {
        ...
        var containerSlot1 = item.FromPackSlot != byte.MaxValue
            ? ModernVersion.AdjustInventorySlot(item.FromPackSlot)
            : item.FromPackSlot;
        var slot1 = item.FromPackSlot == byte.MaxValue
            ? ModernVersion.AdjustInventorySlot(item.FromSlot)
            : item.FromSlot;
        var containerSlot2 = item.ToPackSlot != byte.MaxValue
            ? ModernVersion.AdjustInventorySlot(item.ToPackSlot)
            : item.ToPackSlot;
    ...
    private void HandleSwapItem(SwapItem item)
    {
        ...
        var containerSlotB = item.ContainerSlotB != byte.MaxValue
            ? ModernVersion.AdjustInventorySlot(item.ContainerSlotB)
            : item.ContainerSlotB;
        var slotB = item.ContainerSlotB == byte.MaxValue ? ModernVersion.AdjustInventorySlot(item.SlotB) : item.SlotB;
        var containerSlotA = item.ContainerSlotA != byte.MaxValue
            ? ModernVersion.AdjustInventorySlot(item.ContainerSlotA)
            : item.ContainerSlotA;
    ...
    private void HandleDestroyItem(DestroyItem item)
    {
        ...
        var containerSlot = item.ContainerId != byte.MaxValue
            ? ModernVersion.AdjustInventorySlot(item.ContainerId)
            : item.ContainerId;
    ...
    private void HandleAutoStoreBagItem(AutoStoreBagItem item)
    {
        ...
        var srcBag = item.ContainerSlotA != byte.MaxValue
            ? ModernVersion.AdjustInventorySlot(item.ContainerSlotA)
            : item.ContainerSlotA;
        packet.WriteUInt8(srcBag);
        packet.WriteUInt8(item.SlotA);
        var dstBag = item.ContainerSlotB != byte.MaxValue
            ? ModernVersion.AdjustInventorySlot(item.ContainerSlotB)
            : item.ContainerSlotB;
    ...
    private void HandleAutoEquipItem(AutoEquipItem item)
    {
        ...
        var containerSlot = item.PackSlot != byte.MaxValue
            ? ModernVersion.AdjustInventorySlot(item.PackSlot)
            : item.PackSlot;
    ...
    private void HandleSetDungeonDifficulty(SetDungeonDifficulty difficulty)
    {
        ...
        uint dificultyId = (byte)Enum.Parse(typeof(DifficultyLegacy),
            ((DifficultyModern)difficulty.DifficultyID /*cast due to .constrained prefix*/).ToString());
    ...
    private void HandleBattlenetRequest(BattlenetRequest request)
    {
        ...
        {
            _bnetRpc.Invoke(0u, (OriginalHash)request.Method.GetServiceHash(), request.Method.GetMethodId(),
                request.Method.Token, new CodedInputStream(request.Data));
        }
    }
    ...
    private void WriteSpellTargets(SpellTargetData target, SpellCastTargetFlags targetFlags, WorldPacket packet)
    {
        ...
        if (targetFlags.HasFlag(SpellCastTargetFlags.TradeItem) &&
            target.Item == WowGuid128.Create(HighGuidType703.Uniq, 10uL))
        {
    ...
    private void HandleCastSpell(CastSpell cast)
    {
            ...
            _herbalismProficiencySpells.Contains(cast.Cast.SpellID))
        {
            ...
            if ((cast.Cast.Target.Unit == null || cast.Cast.Target.Unit.IsEmpty()) &&
                GetSession().GameState.CurrentInteractedWithGO != null &&
                !GetSession().GameState.CurrentInteractedWithGO.IsEmpty())
            {
            ...
            GameData.AutoRepeatSpells.Contains(cast.Cast.SpellID))
        {
            ...
            {
                castRequest.ServerGUID = WowGuid128.Create(HighGuidType703.Cast, SpellCastSource.Normal,
                    GetSession().GameState.CurrentMapId.Value, cast.Cast.SpellID,
                    10000 + cast.Cast.CastID.GetCounter());
            ...
            castRequest.ServerGUID = WowGuid128.Create(HighGuidType703.Cast, SpellCastSource.Normal,
                GetSession().GameState.CurrentMapId.Value, cast.Cast.SpellID,
                cast.Cast.SpellID + GetSession().GameState.CurrentPlayerGuid.GetCounter());
        ...
        else
        {
            var castRequest2 = new ClientCastRequest
            {
                ...
                ServerGUID = WowGuid128.Create(HighGuidType703.Cast, SpellCastSource.Normal,
                    GetSession().GameState.CurrentMapId.Value, cast.Cast.SpellID, 10000 + cast.Cast.CastID.GetCounter())
            };
            if (GetSession().GameState.CurrentClientNormalCast != null)
            {
                ...
                else if (GetSession().GameState.CurrentClientNormalCast.Timestamp + 10000 < castRequest2.Timestamp)
                {
                    Log.Print(LogType.Warn,
                        $"Clearing CurrentClientNormalCast because of 10 sec timeout! (oldSpell:{GetSession().GameState.CurrentClientNormalCast.SpellId} newSpell:{castRequest2.SpellId})",
                        "SpellHandler.cs");
        ...
        if (cast.Cast.SpellID == 6478 && (cast.Cast.Target.Unit == null || cast.Cast.Target.Unit.IsEmpty()) &&
            GetSession().GameState.CurrentInteractedWithGO != null &&
            !GetSession().GameState.CurrentInteractedWithGO.IsEmpty())
        {
            cast.Cast.Target.Unit = GetSession().GameState.CurrentInteractedWithGO;
        ...
        Log.Print(LogType.Debug,
            $"[CastSpell] SpellID={cast.Cast.SpellID} TargetFlags=0x{(uint)targetFlags:X} ModernFlags=0x{(uint)cast.Cast.Target.Flags:X} Unit={cast.Cast.Target.Unit} Item={cast.Cast.Target.Item}",
            "");
    ...
    private void HandlePetCastSpell(PetCastSpell cast)
    {
        ...
        if (GetSession().GameState.CurrentClientPetCast != null)
        {
            ...
            {
                Log.Print(LogType.Warn,
                    $"Clearing CurrentClientPetCast because of 10 sec timeout! (oldSpell:{GetSession().GameState.CurrentClientPetCast.SpellId} newSpell:{castRequest.SpellId})",
                    "SpellHandler.cs");
    ...
    private void HandleUseItem(UseItem use)
    {
        ...
        var castRequest = new ClientCastRequest
        {
            ...
            ServerGUID = WowGuid128.Create(HighGuidType703.Cast, SpellCastSource.Normal,
                GetSession().GameState.CurrentMapId.Value, use.Cast.SpellID, 10000 + use.Cast.CastID.GetCounter()),
            ItemGUID = use.CastItem
        };
        Log.Print(LogType.Debug,
            $"[UseItem] SpellID={use.Cast.SpellID} PackSlot={use.PackSlot} Slot={use.Slot} ItemGUID={use.CastItem} PendingCast={GetSession().GameState.CurrentClientNormalCast != null}",
            "");
        if (GetSession().GameState.CurrentClientNormalCast != null)
        {
            if (GetSession().GameState.CurrentClientNormalCast.HasStarted)
            ...
            {
                Log.Print(LogType.Warn,
                    $"Clearing CurrentClientNormalCast because of 10 sec timeout! (oldSpell:{GetSession().GameState.CurrentClientNormalCast.SpellId} newSpell:{castRequest.SpellId})",
                    "SpellHandler.cs");
                SendCastRequestFailed(GetSession().GameState.CurrentClientNormalCast, isPet: false);
                GetSession().GameState.CurrentClientNormalCast = null;
                foreach (var pending in GetSession().GameState.PendingClientCasts)
        ...
        else
        {
            ...
            var containerSlot = use.PackSlot != byte.MaxValue
                ? ModernVersion.AdjustInventorySlot(use.PackSlot)
                : use.PackSlot;
    ...
    private void HandleSupportTicketSubmitComplaint(SupportTicketSubmitComplaint complaint)
    {
        ...
        {
            Session.SendHermesTextMessage(
                "Unable to report player because CharacterName was not resolved (can be fixed by restarting the client)",
                isError: true);
            return;
        }
    ...
    private void HandleActivateTaxi(ActivateTaxi taxi)
    {
        ...
        {
            var path = GetTaxiPath(GetSession().GameState.CurrentTaxiNode, taxi.Node,
                GetSession().GameState.UsableTaxiNodes);
            if (path.Count <= 1)
            {
                return;
            }
    ...
    private void HandleSetTradeItem(SetTradeItem trade)
    {
        ...
        packet.WriteUInt8(trade.TradeSlot);
        var containerSlot = trade.PackSlot != byte.MaxValue
            ? ModernVersion.AdjustInventorySlot(trade.PackSlot)
            : trade.PackSlot;
        var slot = trade.PackSlot == byte.MaxValue
            ? ModernVersion.AdjustInventorySlot(trade.ItemSlotInPack)
    ...
    public override void ReadHandler(SocketAsyncEventArgs args)
    {
        ...
        while (currentReadIndex < args.BytesTransferred)
        {
            ...
            {
                var readHeaderSize = Math.Min(args.BytesTransferred - currentReadIndex,
                    _headerBuffer.GetRemainingSpace());
            ...
            if (_packetBuffer.GetRemainingSpace() > 0)
            {
                var readDataSize = Math.Min(args.BytesTransferred - currentReadIndex,
                    _packetBuffer.GetRemainingSpace());
            ...
            switch (result)
            {
                ...
                case ReadDataHandlerResult.Ok:
                    continue;
                case ReadDataHandlerResult.Error:
                    break;
                default:
    ...
    private ReadDataHandlerResult ReadData()
    {
        ...
        {
            Log.Print(LogType.Error,
                $"WorldSocket.ReadData(): client {GetRemoteIpAddress()} failed to decrypt packet (size: {header.Size})",
                "WorldSocket.cs");
        ...
        {
            Log.Print(LogType.Error,
                $"WorldSocket.ReadHeaderHandler(): client {GetRemoteIpAddress()} sent malformed packet (size: {header.Size})",
                "WorldSocket.cs");
            return ReadDataHandlerResult.Error;
        }
        ...
        switch (opcode)
        {
            case Opcode.CMSG_PING:
            {
                ...
                if (_connectType == ConnectionType.Realm && GetSession().WorldClient != null &&
                    GetSession().WorldClient.IsConnected() && GetSession().WorldClient.IsAuthenticated())
                {
    ...
    public void HandlePacket(WorldPacket packet)
    {
        ...
        Log.PrintNet(LogType.Warn, LogNetDir.C2P,
            $"No handler for opcode {universalOpcode} ({packet.GetOpcode()}) (Got unknown packet from ModernClient)",
            "WorldSocket.cs");
    ...
    private void SendPacketToServer(WorldPacket packet, Opcode delayUntilOpcode = Opcode.MSG_NULL_ACTION)
    {
        ...
        Log.Print(LogType.Error,
            $"Attempt to send opcode {packet.GetUniversalOpcode(isModern: false)} ({packet.GetOpcode()}) while WorldClient is disconnected!",
            "WorldSocket.cs");
    }
        ...
        try
        {
            ...
            catch (AbandonedMutexException ex)
            {
                Log.PrintNet(LogType.Error, LogNetDir.P2C,
                    $"Send mutex was abandoned while sending {packet.GetUniversalOpcode()} ({packet.GetOpcode()}). Continuing with recovered lock. Error: {ex.Message}",
                    "WorldSocket.cs");
            }
            ...
            if (
                universalOpcode is Opcode.SMSG_UPDATE_OBJECT
            )
            {
                var updateCount = data.Length >= 4 ? BitConverter.ToUInt32(data, 0) : 0;
                var mapId = data.Length >= 6 ? BitConverter.ToUInt16(data, 4) : (ushort)0;
                var fingerprint = ZLib.adler32(2552748273u, data, (uint)data.Length);

                // TODO: investigate, why sometimes it works, when some SMSG_UPDATE_OBJECT are ignored
                ...
                // };
                //
                // if (banned.Contains($"{fingerprint:X8}"))
                // {
                //     return;
                // }

                var previewBytes = BitConverter.ToString(data, 0, Math.Min(32, data.Length));
                Log.PrintNet(LogType.Warn, LogNetDir.P2C,
                    $"[CrashTrace] SMSG_UPDATE_OBJECT size={data.Length}, map={mapId}, updates={updateCount}, adler32={fingerprint:X8}, preview={previewBytes}",
                    "WorldSocket.cs");
            }
            ...
            {
                Log.PrintNet(LogType.Warn, LogNetDir.P2C,
                    $"Dropping packet {universalOpcode} - missing modern opcode mapping! (size={data.Length})",
                    "WorldSocket.cs");
                MissingOpcodeTracker.LogDroppedSMSG(universalOpcode, data.Length);
                return;
            }

            Log.PrintNet(LogType.Debug, LogNetDir.P2C, $"Sending opcode {universalOpcode} ({opcode}), size={data.Length}.",
                "WorldSocket.cs");
            ...
            if (packetSize > 1024 && _worldCrypt.IsInitialized && ModernVersion.ExpansionVersion < 3)
            {
                ...
                buffer.WriteUInt32(ZLib.adler32(ZLib.adler32(2552748273u, BitConverter.GetBytes(opcode), 2u), data,
                    (uint)packetSize));
                byte[] compressedData;
    ...
    public uint CompressPacket(byte[] data, ushort opcode, out byte[] outData)
    {
        ...
        Log.PrintNet(LogType.Error, LogNetDir.P2C,
            $"Can't compress packet data (zlib: deflate) Error code: {zRes} msg: {_compressionStream.msg}",
            "WorldSocket.cs");
        return 0u;
    }
    ...
    private void HandleAuthSessionCallback(AuthSession authSession)
    {
        ...
        if (buildInfo == null)
        {
            ...
            Log.Print(LogType.Error,
                $"WorldSocket.HandleAuthSessionCallback: Missing auth seed for realm build {session.Build} ({GetRemoteIpAddress()}).",
                "WorldSocket.cs");
            CloseSocket();
            session.OnDisconnect();
            return;
        ...
        {
            Log.Print(LogType.Error,
                $"WorldSocket.HandleAuthSession: Unknown OS for account: {session?.GameAccountInfo.Id} ('{authSession.RealmJoinTicket}') address: {address}",
                "WorldSocket.cs");
            CloseSocket();
            session?.OnDisconnect();
            return;
        ...
        if (platformSeed == null || !TrySeed(platformSeed))
        {
            ...
            {
                Log.Print(LogType.Warn,
                    $"WorldSocket.HandleAuthSession: Seed mismatch for account: {session.GameAccountInfo.Id} ('{authSession.RealmJoinTicket}') - BYPASSING for testing",
                    "WorldSocket.cs");
            }
        }
        ...
        Log.Print(LogType.Server,
            $"WorldSocket:HandleAuthSession: Client '{authSession.RealmJoinTicket}' authenticated successfully from {address}.",
            "WorldSocket.cs");
    ...
    private void HandleAuthContinuedSessionCallback(AuthContinuedSession authSession)
    {
        ...
        {
            Log.Print(LogType.Error,
                $"WorldSocket.HandleAuthContinuedSession: Authentication failed for account: {accountId} ('{login}') address: {GetRemoteIpAddress()}",
                "WorldSocket.cs");
            CloseSocket();
        }
        else
    ...
    private void HandleEnterEncryptedModeAck()
    {
        ...
        if (_connectType == ConnectionType.Realm)
        {
            ...
            GetSession().AccountDataMgr = new AccountDataManager(GetSession().Username,
                GetSession().RealmManager.GetRealm(_realmId).Name);
    ...
    public void SendAuthResponse(BattlenetRpcErrorCode code, uint queuePos = 0u)
    {
        ...
        if (code == BattlenetRpcErrorCode.Ok)
        {
            ...
            response.SuccessInfo.VirtualRealms.Add(new VirtualRealmInfo(realm.Id.GetAddress(), isHomeRealm: true,
                isInternalRealm: false, realm.Name, realm.NormalizedName));
    ...
    public void SendAccountDataTimes()
    {
        ...
        for (var i = 0; i < count; i++)
        {
            accountData.AccountTimes[i] = GetSession().AccountDataMgr.Data[i] != null
                ? GetSession().AccountDataMgr.Data[i].Timestamp
                : 0;
        }
    ...
    public void SendRpcMessage(uint serviceId, OriginalHash service, uint methodId, uint token,
        BattlenetRpcErrorCode status, IMessage? message)
    {
    ...
    public void InitializePacketHandlers()
    {
        ...
        foreach (var methodInfo in methods)
        {
            foreach (var msgAttr in methodInfo.GetCustomAttributes<PacketHandlerAttribute>())
            {
                ...
                {
                    Log.Print(LogType.Error,
                        $"Tried to override OpcodeHandler of {_clientPacketTable[msgAttr.Opcode]} with {methodInfo.Name} (Opcode {msgAttr.Opcode})",
                        "WorldSocket.cs");
                }
                else
                {
